 ...
 *
 * **Одно объявление на панель и на слой оформления** (его заказ 2026-09-20,
 * пункт 14): панель этим ключом пишет вид значения, а слой им же его ищет —
 * и разойтись им не на чем. В таблице Values ссылка хранится голым именем
 * (`test1`), в строке заметки стоит `[[test1]]`, а решётку от старой формы
 * снимаем: в конфиге заказчика у поля-ссылки приставка `#` осталась от общей
 * формы поля и значением не является.
 */
...
/**
 * Как приставка поля склеивается со значением. **Одно объявление на весь
 * плагин** — по его слову 2026-09-15: «убирать лишнее в самом правиле».
 *
 * До этого дня объявлений было **четыре**, и ни одна пара не отвечала
 * одинаково: 73 расхождения из 136 пар «его приставка × его значение»
 * (`node tools/form_divergence.js`). Три тела из четырёх на его собственной
 * паре — приставка `#`, значение `#todo` — давали **`##todo`**. На экране
 * этого не было видно только потому, что охрана «это уже готовый тег» стояла
 * **у звавших**, ранним возвратом, а не в самом правиле: шесть мест вызова
 * были закрыты, седьмое сломало бы это молча (10.13.147).
 *
 * Правило целиком, и каждая строка — ответ, который раньше давало **какое-то
 * одно** из четырёх тел:
 *
 * - пусто на входе — пусто на выходе. Приставка сама по себе значением не
 *   становится: это и была разница с телом в объекте зависимостей панели,
 *   которое на пустом токене возвращало одну приставку;
 * - готовая ссылка `[[имя]]` проходит насквозь. Приставка — марка **тега**, и
 *   к чужой разметке отношения не имеет;
 * - приставка уже стоит в начале значения — не удваивать. Это и есть его
 *   решение, и здесь же кончается `##todo`;
 * - приставки нет вовсе, а значение начинается с `/` — ставится решётка:
 *   `/1` без этого не стал бы тегом важности ни у кого.
 *
 * **Чего здесь нарочно нет.** Прежнее тело у помощников пропускало насквозь
 * **любой** готовый тег, то есть при приставке `@` и значении `#todo` не
 * применяло приставку вовсе. Правило это уже, чем «не удваивать», и у
 * заказчика неразличимо: приставка у него одна — решётка. Оставить его значило
 * бы, что у поля с приставкой `@` часть значений молча теряет её; снято
 * нарочно, и мера этот край показывает отдельной строкой.
 */
