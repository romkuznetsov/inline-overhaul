 ...
 */
/*
 * **Разбор уже готового токена**, а не нормализация ввода человека: знак
 * списка сюда не приезжает и не снимается. Тот, второй вопрос живёт в
 * `order_deep_editor_state.js` и с 2026-09-15 называется своим именем —
 * `normalizeCheckboxInput`. До того дня оба носили это имя и расходились на
 * восьми входах из 31; ответ заказчика на В-118 («плагин нормализует ввод»)
 * развёл их окончательно. Разбор — PRD 10.13.144 и 10.13.153, правило 117.
 */
...
/**
 * **Как строка делится на зоны — вопрос один, и дом у него один.**
 *
 * Здесь стояло второе объявление, и оно было наивнее первого: единственный
 * разделитель в строке оно считало **первым**, то есть всё, что человек
 * написал после него, объявляло текстом. У заказчика оба разделителя записаны
 * одинаково (`::`), и `- [ ] 11 :: #/1 #todo` для доводки означало «слева
 * `- [ ] 11`, текст `#/1 #todo`, правый Block пуст». Дом (`splitSegments`)
 * отвечает на ту же строку по-другому и верно: единственный разделитель — это
 * **второй** (У-153), значит слева слово человека, а справа его правый Block.
 *
 * Цена ошибки — его замечание `G3` от 2026-09-16: команда элемента вырывала
 * значения правого Block из зоны значений и приклеивала их к тексту, и
 * следующее нажатие растаскивало их по строке — `- [ ] 11 [[test]] ::  ::
 * #/1 #todo …` вместо `- [ ] 11 :: #/1 #todo [[test]] …`.
 *
 * Расхождение измерено **до** правки на его заметках: 24 строки из 67, где
 * разделитель вообще есть, и все 24 — ровно этот случай (мера —
 * `tools/line_matrix.js`, сводка второго нажатия).
 *
 * **Панель этот дом уже зовёт:** `tagwheel.js` подаёт `splitSegments` под
 * именем `splitThreeSegments` в набор зависимостей доводки. То есть дороги
 * расходились не задумкой, а тем, что одна из них так и не переехала.
 *
 * Разделители остаются в списке аргументов: их передают десять мест, и все
 * десять берут их у тех же правил. Дом спрашивает правила сам.
 *
 * **Форма ответа остаётся прежней, и это не мелочь.** Дом уносит цитату и
 * каллаут в `indent` (10.13.118), а здесь они всегда лежали в левой зоне, и
 * десять звавших читают её именно так. Отданные домом, они делали левую зону
 * пустой — а сборка на пустой левой зоне подставляет знак списка, то есть наш
 * знак вставал внутрь чужого начала строки: `>  -  :: #/1`. Это поймал обход
 * строки и проверка поведения, а не чтение. Поэтому меняется **ответ на вопрос
 * о зонах**, а не форма: внешнее оформление возвращается в левую зону тем же
 * куском, каким дом его снял.
 */
function splitThreeSegments(line, rules, sep1, sep2) {
  ...
  /*
   * **Дом спрашивается там, где position не отвечает** (У-153). Пока
   * разделители записаны по-разному, место разделителя в строке и есть ответ:
   * первый — первый, второй — второй, и резать можно прямо. Совпали — и
   * позиция не знает ничего, а дом знает.
   *
   * Граница у делегирования не осторожности ради: у дома есть своя догадка
   * для случая «второго разделителя в строке нет» — хвост, похожий целиком на
   * правый Block, он правым Block и объявляет. При разных разделителях это
   * ровно то, на что заказчик приносил замечание 2026-09-13 («была строка
   * `- 12`, после активации Due получил `-  :: 📅… 12`»), и проверка,
   * купленная тем замечанием, краснеет на такой ширине. Поэтому здесь
   * меняется только тот случай, где прежний ответ был **угадан**, а не
   * прочитан.
   */
  if (s1 && s1 === s2) {
...
function normalizeSingleSeparatorLayout(line, rules) {
  ...
  /*
   * **Пустой слот под текст держится у любого знака начала строки, а не у
   * одного дефиса** (10.13.156).
   *
   * Слот — это два пробела между началом строки и разделителем: человек
   * видит, куда встанет слово (10.13.34). Условие здесь было написано
   * литералом `left === "-"`, то есть знало **одну** форму начала из пяти. На
   * строке-заголовке слот схлопывался в один пробел, и команда отдавала
   * `# :: 📅…` там, где панель отдавала `#  :: 📅…`, — снова «панель и
   * команда дают разное», и снова невидимо, пока не спросишь обход.
   *
   * Спрашивается это у общего объявления начала строки: слева стоит только
   * начало и ничего больше (У-177 — вопрос о предмете, а не о вместилище).
   */
...
function normalizeStructuredSlots(options) {
  ...
  const leftWithoutList = String(seg.left || "")
    ...
    .replace(/^\s*\[[^\]]\]\s+/, "");
  /*
   * **Граница зоны — это тот же пробел.** Знак заголовка у Obsidian есть знак
   * **и пробел** за ним (З3 № 110), и `extractHeadingPrefix` спрашивает именно
   * это. Пока деление на зоны отдавало сырые куски строки, пробел перед
   * разделителем приезжал сюда сам; дом отдаёт содержимое зоны без него, и
   * левая зона `- ##` переставала быть заголовочной — строка `- ## :: 111`
   * оставалась как есть вместо `## 111`. Поэтому граница зоны подставляется
   * явно: в самой строке на её месте и стоял пробел.
   */
   ...
   * (10.13.156).
   *
   * Правило здесь одно: на строке-заголовке, у которой зона значений пуста,
   * висящему разделителю взяться неоткуда — `#  :: слово` собирается как
   * `# слово`. Но «слово» спрашивалось **по месту в строке**, а не по тому,
   * что это за текст, и у заказчика в этот слот попадает правый Block: у него
   * оба разделителя записаны одинаково (`::` и `::`), и деление на три зоны
   * второго разделителя не находит — весь правый Block читается как текст.
   *
   * Цена ошибки видна на экране: команда поля на строке `# #aaa` отдавала
   * `# #work #aaa` — значения приклеены к заголовку, разделителя и слота под
   * текст нет, — а панель на той же строке отдавала `# #work #aaa :: `. Это
   * та самая разница «панель и команда дают разное»; на разведённых
...
function enforceRightPayloadSeparatorInvariant(options) {
  ...
  if (rawPlain) {
    ...
    /*
     * **Спрашивается удвоенный текст, а не зона, в которой он лежал.** Правило
     * про удвоение написано о слове человека: оно сравнивает написанное им до
     * действия с тем, что стоит в строке после. В какую зону это слово
     * попадёт, решает деление строки, и при совпадающих разделителях оно
     * теперь отвечает домом: `- 11 11 :: ➕…` делится как «знак списка / текст
     * `11 11` / правый Block», а прежде — как «левая зона `- 11 11` / …».
     * Пока вопрос задавался одной зоне, ответ зависел от того, куда предмет
     * положили (У-177).
     */
