 ...
 *
 * Положений четыре. Три — его слово 2026-09-19: `After parent` — поле
 * появляется после значения родителя, так было всегда, и это умолчание;
 * `Show always` — оно работает и на строке без родителя, а команды листают все
 * его значения подряд; `Hide` — поля нет ни в TagWheel, ни в его командах.
 * Четвёртое — `Show when press Alt` (`З-36`, его пункт 11 от 2026-09-22):
 * в tagWheel поле видно, только пока на родителе зажат `Alt`; команды его
 * работают, как при `After parent`.
 *
 * **Умолчание стоит первым** — его пункт 12, 2026-09-22: «в io-field-child
 * сделай дефолтным полем After parent». Поведение таким и было (`getSubMode`
 * без записей отвечает `after-parent`), а список открывался с `Show always`,
 * и первая строка списка читается как стандартная. Так же устроены соседние
 * списки этого блока: у `Active` первым стоит `Yes`, у `Behavior` — `Strict`.
 *
