...
/* ---- Г1–Г5: слой настроек по исходникам -------------------------------- */
{
  ...
  const ALLOW: ReadonlyArray<{ gate: string; at: string; why: string }> = [
    ...
    {
      gate: "Г1",
      at: "custom/theme_colors.ts",
      why: "это не вид, а вопрос браузеру: узел с `color: var(--…)` добавляют, спрашивают и снимают внутри одного вызова, чтобы тема ответила посчитанным цветом, а не своим текстом",
    },
    {
      gate: "Г2",
 ...
 */
{
   ...
   */
  /*
   * С 2026-09-08 колонки держит **сетка**, а не флекс (решение заказчика по
   * В-92): у них появилась общая строка шапки, и подсказка раскрывается под
   * ней во всю ширину редактора, а не внутри колонки шириной 188 точек.
   *
   * Утверждение переехало за предметом (У-94), и гарантия у него та же:
   * дорожек ровно две, переносить нечего, правая берёт остаток и умеет
   * сжаться до нуля. `minmax(0, 1fr)` — это и есть «остаток и до нуля»:
   * без нуля содержимое правой вытолкнуло бы список Fields.
   */
  const tracks = /\.io-fields\s*\{[^}]*grid-template-columns:\s*([^;]+);/.exec(css);
  const trackList = tracks ? String(tracks[1]).trim() : "";
  if (!/display:\s*grid/.test(/\.io-fields\s*\{[^}]*\}/.exec(css)?.[0] || "")) {
    fail("редактор Fields больше не сетка: у колонок пропала общая строка шапки (В-92)");
  } else if (!/^188px\s+minmax\(\s*0\s*,\s*1fr\s*\)$/.test(trackList)) {
    fail("дорожки редактора Fields не те: ждали `188px minmax(0, 1fr)`, стоит `" + trackList + "`");
  } else ok("две колонки редактора Fields стоят рядом всегда, правая сжимается до нуля");

  /* Строка подсказки идёт на обе колонки — ради неё сетку и заводили. */
  if (!/\.io-fields__tiprow\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/.test(css)) {
    fail("подсказка редактора Fields больше не во всю ширину: вернулся дефект В-92");
  } else ok("подсказка редактора Fields раскрывается во всю его ширину");
...
/* ---- Г16: дымовой тест — каждая вкладка и каждый свой блок ------------- */
{
   ...
   * рисуется, и Г16 проверял бы пустое место.
   *
   * **Подделки нормализации Order здесь больше нет.** Она лежала рядом с этим
   * объяснением — «нормализация, доводящая форму до полной, как настоящая», —
   * и не звалась ниоткуда: платформа ниже берёт настоящую,
   * `internals.normalizePkmOrder`. То есть объяснение описывало подделку,
   * которая ничего не делала, а гейт при этом гонял настоящий код (У-71).
   * Снято 2026-09-08.
   */
  /*
...
/* ---- Бюджет производительности (раздел 12) ----------------------------- */
{
  ...
  /* ---- П1: цена открытия вкладки ---------------------------------------- */
  {
    ...
    /*
     * **Положительный контроль замера** (У-88). `drawn` считал отрисованные
     * блоки и никем не читался: то есть замер мог мерить пустоту — ровно то,
     * от чего предостерегает объяснение выше, и ровно то, что с ним однажды
     * уже случилось. Число прогретого прохода и есть доказательство, что
     * работа под замером была.
     */
    if (!drawn) fail("бюджет: за прогревом не отрисовано ни одного своего блока — замер мерит пустоту");
    const warmed = drawn;
    ...
    if (slow.length) {
      ...
      ok("бюджет: наша часть открытия вкладки — " + times.join(", ")
        + "; своих блоков под замером " + (drawn - warmed));
    }
  }
