...

{
   ...
   *
   * Здесь стояла ссылка на `getBoundHotkeyForCommand` в `main.js` — та
   * пара функций снята 2026-09-07: её не звал никто, а правило «как
   * читается запись хоткея» объявлено в `bindingKey`
   * (`settings_backup.js`) и здесь. Двух объявлений и хватит (У-32).
   */
  const plugin = {
    manifest: { id: "inline-overhaul" },
    app: {
