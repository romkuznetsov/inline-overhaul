...
- **П-4. Поиск — платформенный.** Глобальный поиск Obsidian индексирует декларативные вкладки, поэтому своего поля поиска мы не делаем (ответ на находку К2 аудита). Установлено по документации 2026-08-24: **отдельного поля ключевых слов у определения нет**, поиск работает по видимому тексту, а у строки есть `searchable` для отказа от индексации. Следствия:
  - `searchTerms` попадают в тот же `DocumentFragment`, что и `desc`, отдельной приглушённой строкой `Previously called <старое имя>`. Линтер текстов (Г10) эту фразу разрешает, и она же служит читателю: он видит, что настройка переименована;
