...
/**
 * Шапка таблицы настроек — **по столбцам, а не по написанию**.
 *
 * Obsidian выравнивает таблицы сам: стоило заказчику открыть заметку, как
 * шапка стала `| Вкладка    | Контрол   | Значение |`, и признак по точной
 * строке её не узнал — сторож объявил дефектом собственное форматирование
 * платформы. Сравниваются имена столбцов со снятыми пробелами (У-201).
 */
const TABLE_COLUMNS = ["Вкладка", "Контрол", "Значение"];

function isSetupTableHead(line) {
  const cells = String(line || "").split("|").map((c) => c.trim()).filter((c) => c !== "");
  if (cells.length !== TABLE_COLUMNS.length) return false;
  return TABLE_COLUMNS.every((name, i) => cells[i] === name);
}

const LIVE_LEAD = "Ваша строка, работайте на ней";
const LIVE_NONE = "Живой строки здесь нет";
const SAMPLE_CALLOUT = "> [!check]";
...
/**
 * Строки, изображающие строку заметки: эталон в рамке и рабочая строка.
 *
 * Эталон — тело коллаута `[!check]`, от его заголовка до первой строки не из
 * коллаута. Рабочая строка — голая строка списка раздела. Всё остальное —
 * обычный текст и журнал, и к ним требование «без обратных кавычек» не
 * относится: они называют файлы и настройки по именам.
 */
function sampleLinesOf(check) {
  const out = [];
  let inside = false;
  for (const line of check) {
    if (line.indexOf(SAMPLE_CALLOUT) === 0) { inside = true; continue; }
    if (inside) {
      if (line.indexOf(">") !== 0) inside = false;
      else if (/- /.test(line)) out.push(line);
      continue;
    }
...
function troublesOf(test) {
  ...
  if (!setup) {
    ...
    if (!setup.some((l) => isSetupTableHead(l))) {
  ...
  if (!check) {
     ...
     */
    /*
     * Предмет — **строки эталона и рабочая строка**, а не весь раздел: журнал
     * стоит тем же коллаутом ниже и имена файлов носит законно. Первая версия
     * признака этой границы не знала и объявила дефектом четыре записи журнала.
     */
    for (const line of sampleLinesOf(check)) {
      if (line.indexOf("`") !== -1) {
    ...
    /*
     * **Голая строка списка в разделе ровно одна — его собственная.**
     *
     * 2026-09-20 он тестировал прямо в заметке, и плагин записал значения в
     * строки эталона и в блок «Действие»: для плагина любая строка заметки —
     * рабочая, и отличить эталон от рабочей строки он не может. Эталон поэтому
     * живёт внутри коллаута (строки начинаются с `>`), а голой остаётся ровно
     * одна — та, на которой он и должен нажимать. Где проверка идёт не на
     * строке, голых нет вовсе.
     */
    const bare = check.filter((l) => /^- /.test(l));
    const want = check.some((l) => l.indexOf(LIVE_LEAD) !== -1) ? 1 : 0;
    if (bare.length !== want) {
      out.push("голых строк списка в разделе " + bare.length + ", а должно быть " + want
        + ": эталон живёт в коллауте, иначе плагин пишет в него наравне с рабочей строкой");
    }
    if (want === 1 && !check.some((l) => l.indexOf(SAMPLE_CALLOUT) === 0)) {
      out.push("эталон не в коллауте — он не отличим от рабочей строки");
    }
  }
...
function selfCheck() {
  ...
  const want = [
    ...
    "таблицы по вкладкам", "по шагам", "голых строк списка", "не в коллауте",
