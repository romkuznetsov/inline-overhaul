...
{
  /*
   * **Разделитель в предпросмотре — человека, а не из кода** (У-186).
   *
   * Десять мест читали настройку и писали рядом запасное `"||"`. Проверка
   * спрашивает обе стороны: нарисован ли выбранный человеком знак и **не
   * нарисован** ли при этом прежний литерал. Разделители разведены нарочно —
   * на паре одинаковых место, читающее только первый, неотличимо от верного
   * (У-147).
   */
  const host = makeNode("div");
  const close = linePreview(host as unknown as El, makeCtx(null, {
  ...
  const seps = texts(host, "io-line__sep");
  assert.ok(seps.includes("::"), "первый разделитель человека нарисован: " + seps.join(" | "));
  assert.ok(seps.includes("~~"), "второй разделитель человека нарисован: " + seps.join(" | "));
  assert.ok(!seps.includes("||"), "умолчания из кода в предпросмотре нет: " + seps.join(" | "));
  close();
  ok("разбор строки: разделители из настроек, а не из кода");
}

{
  /*
   * **И вторая половина того же вопроса — та, которую видит подмена.**
   *
   * Пока значение приезжает непустым, запасное `"||"` рядом с чтением
   * неотличимо от его отсутствия: проверка выше зелена и с ним, и без него.
   * Предмет здесь — случай, когда значение до предпросмотра **не доехало**:
   * с запасным литералом на экране появлялся бы разделитель, которого у
   * человека нет, а без него не появляется ничего. Мутация: вернуть
   * `readText(ctx, SEP1_PATH, "||")` — и это утверждение краснеет.
   */
  const host = makeNode("div");
  const close = linePreview(host as unknown as El, makeCtx(null, {
    "pkm.lineFormat.separator1": "",
  ...
  const seps = texts(host, "io-line__sep").filter(t => String(t).trim());
