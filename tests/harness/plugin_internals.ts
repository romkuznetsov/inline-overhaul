...
export interface PluginInternals {
  ...
  /* Гиперссылки строки — его заказ `В-181`. Разбор отдельный от токенов
     нарочно: он ничего не решает о Block, только о двух цветах. */
  scanHyperlinksInLine: (text: string) => Any[];
