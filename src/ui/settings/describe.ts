 ...
 * **Прежних имён здесь нет, и это не пропуск.** В шапке стояло обратное —
 * «строка „Previously called …“ для переименованных настроек: отдельного поля
 * ключевых слов у платформы нет, а поиск идёт по видимому тексту», — и оба
 * утверждения оказались неверны. Поле у платформы есть: `aliases`, и
 * `to_definitions.ts` кладёт туда `searchTerms` схемы с самого начала. А сама
 * строка не рисовалась никогда: шов `previouslyCalled` объявлен был, и его не
 * читал даже этот файл. Утверждение о состоянии, переставшее быть верным,
 * врёт тише всего (У-64); снято 2026-09-08 вместе со швом.
 *
