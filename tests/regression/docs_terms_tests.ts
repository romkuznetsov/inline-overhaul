...

{
  ...
  const REMOVED: Array<[string, string]> = [
    ...
    /* Переименование видимых имён его словом 2026-09-21 (`З-35`, В-166 и
       В-167). Идентификаторы команд при этом не тронуты — Р3 даёт один разрыв
       совместимости, а не два, — а прежние имена в документе посылают человека
       искать в палитре строку, которой там нет. Запрет снимается, только если
       он вернёт прежние подписи. */
    ["Open TagWheel on the left", "В-166: команда переименована в tagWheel Left"],
    ["Open TagWheel on the right", "В-166: команда переименована в tagWheel Right"],
    ["Jump back", "В-167: команда переименована в Jump up"],
    ["Jump next", "В-167: команда переименована в Jump down"],
    /* Само написание, а не только имена команд: `TagWheel` человек больше
