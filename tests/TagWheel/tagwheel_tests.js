 ...
 * Сам `Ctrl+Z` проверяется с 2026-09-07 — `runUndoAfterPanelSuite` ниже, на
 * настоящей истории CodeMirror. Пока его не было, здесь стояло «пакета с
 * историей в наборе нет, поэтому Ctrl+Z остаётся за глазами»: ровно в том, чего
 * этот шов не видит, и жил дефект A41 (У-98).
 */
...
function runValueNamesSuite(core, baseRules) {
  ...
  /*
   * **Общий дом правила «свой текст рядом с написанным»** — его зовёт и
   * коробка скроллера (его слово 2026-09-21, вечер: «добавь опцию
   * `Custom+Default`… чтобы в скроллере тоже было видно `💡#idea`»). До этой
   * сессии коробка отвечала на тот же вопрос сама, и два объявления разошлись
   * бы молча (У-159).
   *
   * Написанное здесь нарочно **не равно** токену: коробка считает его
   * `formatVisualToken` и на выключенном `Show tag markers` отдаёт слово без
  ...
  assertEq(core.joinValueLabel(PRINTED, 'todo', 'value'), 'todo',
    'при умолчании свой текст не печатается')
  assertEq(core.joinValueLabel('', 'todo', 'both'), 'todo',
    'своего текста нет — печатается написанное, и оно не удваивается')
  assertEq(core.joinValueLabel('   ', 'todo', 'custom'), 'todo',
    'пробелы своим текстом не считаются')
