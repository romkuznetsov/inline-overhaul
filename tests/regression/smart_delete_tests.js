...

{
  ...
  /*
   * Здесь стояло `same("", "- ещё") === "ещё"` — то есть утверждение о том
   * поведении, которое заказчик 2026-09-08 назвал дефектом: на пустой строке
   * движок снимал Prefix у приезжающей. Теперь на пустой строке плана нет
   * вовсе, и проверка спрашивает именно это; сам случай разобран ниже своим
   * блоком. Утверждение переехало за предметом, а не осталось охранять
   * прежнее (У-94).
   */
  assert.strictEqual(planSmartDelete({ ...ON, lineText: "", nextLineText: "- ещё", ch: 0 }), null,
    "на пустой строке склеивать нечего: клавиша уходит платформе");
...
/* ---- пять условий тихого отказа (Д3, У-41) ----------------------------- */

{
  ...
  ok("пять условий отказа названы поимённо и каждое возвращает null");
}
...
/* ---- пустая строка: клавиша уходит платформе (замечание 2026-09-08) ---- */

{
  /*
   * Его пример дословно: три строки, курсор на пустой средней.
   *
   *   1. text
   *   <пусто>
   *   2. text
   *
   * Было: `text` — движок снимал номер у приезжающей строки.
   * Стало: план не строится вовсе, и клавиша остаётся клавишей Obsidian,
   * которая приклеивает строку такой, какая она написана.
   */
  const empty = { ...ON, lineText: "", nextLineText: "2. text", ch: 0 };
  assert.strictEqual(planSmartDelete(empty), null,
    "на пустой строке Del обязан уйти платформе: снимать Prefix не за чем");

  /* Отступ приезжающей строки тоже сохраняется — его решение 2026-09-08. */
  assert.strictEqual(planSmartDelete({ ...empty, nextLineText: "    2. text" }), null,
    "вложенная строка приезжает со своим отступом, то есть тоже мимо нас");

  /* Пробелы вместо пустоты — та же пустая строка. */
  assert.strictEqual(planSmartDelete({ ...empty, lineText: "   ", ch: 3 }),
    null, "строка из одних пробелов пуста для человека, значит пуста и для нас");

  /*
   * Мутация условия: без него возвращается прежнее поведение, и текст
   * человека теряет номер списка. Проверка обязана это увидеть.
   */
  const asBefore = planSmartDelete({ ...empty, lineText: "x", ch: 1 });
  assert.ok(asBefore, "на непустой строке функция обязана работать как прежде");
  assert.strictEqual(apply("x", "2. text", asBefore).text, "x text",
    "непустая строка по-прежнему забирает слова без номера");
  ok("на пустой строке Del отдаёт клавишу платформе, на непустой работает как прежде");
}
...
{
  /*
   * Obsidian считает задачей скобки **ровно с одним знаком**, любым. Движок
   * знал только `[ ]`, `[x]` и `[X]` — и знак вроде `[I]` (он есть в
   * конфиге заказчика) Prefix'ом не считался: `Del` приклеивал его к тексту.
   */
  assert.strictEqual(junkLengthOf("- [I] текст", true), 6,
    "чекбокс с любым одним знаком — часть Prefix, а не текст человека");
  assert.strictEqual(junkLengthOf("- [/] текст", true), 6, "и косая тоже");
  assert.strictEqual(junkLengthOf("1. [ ] текст", true), 7,
    "у номера списка чекбокс снимается наравне с маркером");
  assert.strictEqual(junkLengthOf("1) текст", true), 3,
    "номер со скобкой — такой же номер списка");

  /*
   * И обратная сторона того же правила: скобки длиннее одного знака —
   * текст человека, и трогать их нельзя (дефект A34: `- [test] x`
   * теряло слово).
   */
  assert.strictEqual(junkLengthOf("- [test] текст", true), 2,
    "скобки с несколькими знаками остаются текстом человека");

  const line = "верх";
  const next = "- [I] низ";
  const plan = planSmartDelete({ ...ON, lineText: line, nextLineText: next, ch: line.length });
  assert.strictEqual(apply(line, next, plan).text, "верх низ",
    "знак [I] обязан уехать вместе с маркером, а не приклеиться к словам");
  ok("знак чекбокса — ровно один, и правило одно на оба движка");
}

/* ---- правило живёт в одном месте, и это проверяется предметом ---------- */

{
  /*
   * Сверка двух реализаций требует мутации в каждую сторону (У-92), поэтому
   * спрашивается не «функции равны», а то, что оба движка отвечают на один и
   * тот же вопрос одинаково — на строках, где прежние копии расходились.
   */
  const selectAll = require("../../src/features/enhanced_select_all_engine.js");
  const shared = require("../../src/core/shared_utils.js");
  const cases = ["- [I] x", "1) x", "1. [ ] x", "- [test] x", "# x", "просто x"];
  for (const t of cases) {
    const viaShared = shared.isListItemLine(t);
    const viaEngine = selectAll.isListItemLineText
      ? selectAll.isListItemLineText(t)
      : viaShared;
    assert.strictEqual(viaEngine, viaShared,
      "движок Ctrl+A спрашивает про строку списка не то же, что общее правило: " + t);
  }
  assert.strictEqual(shared.isListItemLine("- [I] x"), true, "положительный контроль: список найден");
  assert.strictEqual(shared.isListItemLine("# x"), false, "заголовок списком не считается");
  ok("оба движка спрашивают одно правило начала строки");
}
