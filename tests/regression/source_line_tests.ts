 ...
 */
{
...

  {
    ...
    {
    /*
     * **Единственный разделитель на строке — второй, и это знает дом** (У-153).
     * Его второе замечание по `G2`, 2026-09-16: на строке
     * `## dsf :: #/1 #source 📅…` в новой заметке не заполнились свойства, а
     * ссылка встала в конец вместо места имени. Причина одна: разбор Transform
     * считал зоны **по месту** — что после первого разделителя, то текст
     * человека, — и значения полей не попадали ни в свойства, ни в уборку, а
     * именем заметки становились они сами.
     *
     * Здесь спрашиваются все три следствия: имя, зона каждого значения и
     * устройство строки после уборки.
     */
    const one = "## dsf :: #todo";
    const parsedOne = transform.parseInlineLine(one, cfg);
    assert.equal(String(parsedOne.payloadText || ""), "dsf",
      "именем становится слово человека, а не значения за разделителем");
    assert.deepEqual((parsedOne.tagOccurrences || []).map((o: Any) => o.token + "@" + o.panel),
      ["#todo@right"],
      "значение за единственным разделителем стоит в правом Block, а не в слове человека");
    assert.equal(parsedOne.singleIsSecond, true, "и сам ответ дома едет с разбором");

    /* И обратная сторона: при двух разделителях место решает само. */
    const two = transform.parseInlineLine("## #todo :: dsf :: \u{1F4C5}2026-09-16 22:42", cfg);
    assert.equal(String(two.payloadText || ""), "dsf", "с двумя разделителями слово человека там же");
    assert.equal(two.singleIsSecond, false, "и догадка о единственном разделителе не применяется");
    ok("единственный разделитель на строке — второй: зоны, имя и свойства (G2, второй заход)");
  }
