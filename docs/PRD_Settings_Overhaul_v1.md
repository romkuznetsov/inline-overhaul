...
SettingDef.tip / seeAlso           →  своя вёрстка внутри desc-области, см. 5.3
SettingDef.searchTerms             →  см. 5.3, пункт П-4
```
...
- **П-4. Поиск — платформенный.** Глобальный поиск настроек Obsidian индексирует декларативные вкладки, поэтому отдельного поля поиска мы не делаем (это ответ на находку К2 аудита). Первое, что проверяет исполнитель в фазе 1: попадают ли в индекс `desc` и есть ли поле ключевых слов для `searchTerms`. Если поля нет — старые имена дописываются в конец `tip` служебной фразой `Previously called <старое имя>`, и линтер текстов (Г10) её разрешает. Требование И1 проверяется поиском в живой панели, а не на словах.
...
- **Н1.** У заголовка каждой группы справа появляется кнопка `Reset group`. Место под неё занято всегда; видна она при наведении на заголовок, при фокусе внутри него и на группе, которую пользователь только что менял. Правило то же, что у стрелок в списке Fields (Ф18): появление не двигает вёрстку.
...
8. **Проверка платформы фактами** и запись результата в этот документ, пункт П-4: попадает ли `desc` в индекс глобального поиска; есть ли поле ключевых слов для `searchTerms`; как ведёт себя поиск со скрытой предикатом настройкой. Если поля ключевых слов нет — включается запасной путь из П-4.
