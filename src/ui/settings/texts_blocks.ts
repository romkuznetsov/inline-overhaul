...
export const BLOCK_TEXTS = {
  ...
  "field-editor": {
    ...
    SIDE_CUSTOM_ABOUT: "Everything in {0} is written where the cursor is, by its own tagWheel command",
    RENAME_BLOCK: "Rename the block {0}",
    DELETE_BLOCK: "Delete the block {0}",
    ...
    BEHAVIOR_TIP: "<b>Strict</b> writes the Value in its own Block and changes the line Prefix. <b>Insert only</b> writes the Value in its own Block and does not change the line Prefix. A Field that goes where the cursor is belongs in a custom block: <code>Add Block</code> under the Fields list",
    BEHAVIOR_FOR: "Behavior for {0}",
    CHILD_NAME: "Child Field",
    CHILD_DESC: "When the child Field is offered: after a parent Value, always, on an Alt press, or never",
    CHILD_TIP: "A child Field is a second Field that only makes sense under this one: its Values are the ones marked child in the table below. <b>After parent</b> offers it once the parent has a Value — that is how it has always worked — and a press of <code>Alt</code> on the parent opens it before that. <b>Always</b> offers it on any line: tagWheel shows it with no parent picked, and its commands step through every child Value in turn. <b>On Alt</b> keeps it out of tagWheel until you press <code>Alt</code> on the parent: it appears next to the parent, so you can step onto it and pick a Value — even before the parent has one, and then the Value stands without a parent. Press <code>Alt</code> again and it hides, and the Value you picked stays on the line. <code>Alt</code> together with another key is a hotkey and does not open it. Its commands work as with <b>After parent</b>. <b>Hide</b> keeps it out of tagWheel and out of its commands even when its Values are set up",
    CHILD_ALWAYS: "Always",
    CHILD_AFTER_PARENT: "After parent",
    CHILD_ALT: "On Alt",
    CHILD_HIDE: "Hide",
    CHILD_OF: "Child Field of {0}",
    CHILD_PARENT_NAME: "Parent Value",
    CHILD_PARENT_DESC: "What happens to the parent when a child Value is set without it",
    CHILD_PARENT_TIP: "Shown only with <b>Always</b>. <b>Child only</b> writes what you picked and nothing else. <b>Add parent Value</b> also writes the parent Value that this child Value belongs to, so the pair on the line is one you set up here",
    CHILD_PARENT_KEEP: "Child only",
    CHILD_PARENT_ADD: "Add parent Value",
    ...
    YAML_NAV_NAME: "YAML of navigator values",
    YAML_NAV_DESC: "Write the navigator of a child Value into this Field's property too",
    YAML_NAV_TIP: "A navigator is never written to the line, so on its own it never reaches the note. On, <code>Inline to note</code> writes the navigator of the child Value into this Field's property, and the child goes into its own property as before. A child under two navigators writes both, as a list. Off, only the child is written",
    YAML_NAV_OF: "YAML of navigator values for {0}",
    ...
    PREREQ_PICK_TIP: "This Field can wait for any other Field, of any type. Two Tag Fields tied this way are also kept next to each other in the line",
    ...
    ELEMENT_COMMAND_TIP: "<code>Current date and time</code> writes the moment of the press in the shape set by <code>Value format</code>. The random ones fill the Value with numbers or letters, which is what an id needs",
    COMMAND_NOW: "Current date and time",
    ...
    NEW_FIELD_TYPE_TIP: "<b>Tag</b> writes a tag such as <code>#todo</code>, <b>Link</b> writes a link such as <code>[[Project A]]</code>, <b>Element</b> writes an emoji with a Value after it, such as <code>📅 2026-08-27</code>. The type stays as you choose it here: it decides what the Field writes into the line",
  ...
  "smart-rules-list": {
    ...
    FOLDER_NEAR_NOTE: "Next to note",
    FOLDER_OTHER: "Other folder…",
    ...
    RULE_ADVANCED_CUSTOM: "Custom",
    PL_ROW_ARIA: "{0} for {1}",
    PL_POSITION: "Where to put the text",
    PL_POSITION_BEGINNING: "Beginning",
    PL_POSITION_END: "End",
    PL_POSITION_HEADER: "Under heading",
    ...
    PL_HEADER_MODE_NONE: "None",
    PL_HEADER_LEVEL: "Line above is a heading",
    PL_HEADER_LEVEL_PLAIN: "Plain text",
