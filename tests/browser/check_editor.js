...
async function main() {
  ...
  try {
     ...
     * точек из сорока.
     *
     * Мерится середина подложки против середины **узла строки** — того, что
     * Obsidian и рисует. Отступ заголовка лежит снаружи узла, и именно на нём
     * расходились блок и написанное: у этой строки блок отдаёт верх `362` при
     * высоте `52`, а узел стоит на `384` и высок на те же `52`. То есть высоту
     * блок считает без отступа, а верх — с ним, и подложка, поставленная по
     * блоку, уезжала вверх на весь отступ.
     *
     * Пузырь тут не годится в эталон: он выровнен по базовой линии крупного
     * кегля и сам стоит ниже середины строки. Спрашивается то, что решает
     * правило продукта, — середина зрительной строки.
     */
    const head = base.heading;
    if (!head) {
      bad("строки-заголовка на странице нет — вертикаль подложки на ней проверять не на чем");
    } else {
      if (!(head.line.height > base.lineHeight + 4)) {
        bad("подделка заголовка ничего не подделала: ящик строки " + head.line.height
          + " точек против умолчания " + base.lineHeight + " — предмета замечания нет");
      }
      ...
      if (!head.token) {
        ...
        const rowMid = (head.line.top + head.line.bottom) / 2;
        for (const b of head.bands) {
          ...
          if (!near(bandMid, rowMid, 1.5)) {
            bad("на строке-заголовке подложка не по середине написанного: её середина "
              + bandMid + ", середина строки " + rowMid + " (разница "
              + Math.round((bandMid - rowMid) * 100) / 100 + ")");
          }
          /* И она не вылезает из строки: прижим по блоку это нарушал. */
