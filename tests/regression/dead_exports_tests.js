...
function codeOnly(src) {
  ...
  while (i < src.length) {
    ...
    if (ch === '"' || ch === "'" || ch === "`") {
      ...
      /*
       * **Кавычка не переживает строку — обратная тоже** (У-139, У-144).
       *
       * Прежде обратной кавычке давали искать пару до конца файла:
       * шаблонная строка бывает многострочной. Но своего разбора
       * регулярных выражений у маски нет, а обратная кавычка **внутри
       * образца** есть — в `navigation_runtime.js` стоит образец с ней.
       * Маска читала её как начало шаблона и съедала остаток файла:
       * обход видел 20 227 знаков из файла втрое длиннее и объявил
       * ничьим живой экспорт, который зовут ниже по тексту.
       *
       * Ошибка неизбежна — полным был бы только разбор языка — и потому
       * ограничена одной строкой. Ценой этого содержимое многострочного
       * шаблона считается кодом: обход может **не заметить** мёртвое имя,
       * но не назовёт мёртвым живое.
       */
