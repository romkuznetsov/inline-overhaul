...
- **З3.** Не трогать файлы `pkm_v2/**`, `navigation_runtime.js`, `pkm_runtime_v2.js`, `src/core/pkm_*`, `src/core/status_*`, `src/core/token_graph_unified.js`, `src/core/line_pipeline.js` — кроме случаев, когда меняется путь чтения настройки из конфига (фаза 2).
  **Исключение, разрешённое заказчиком 2026-08-27** (единственное на сегодня, и записано здесь, чтобы не стало правилом): `reconcileModeDependencies` в `src/core/pkm_rules_runtime_helpers.js` — 53 строки в одной функции. Без них предусловие Field (10.13.4) не работает в том виде, в каком заказчик его заказал: ссылка не может ждать тег, движок стирает связь и выключает Field. Обойти из слоя настроек нечем — все, кто зовёт эту функцию, тоже под З3. Правка сделана в одну сторону (правый список ищет родителя в обоих, левый только у себя), проверена сюитой целиком и закреплена проверкой `tests/regression/prerequisite_runtime_limits_tests.ts` на настоящих функциях движка. Раздел 3.2 при этом остаётся в силе: другие правки поведения движка по-прежнему вне границ.
...
**Проверка плагина в vault, 2026-08-28: шесть находок заказчика.** Ни одна не исправлена — разбор сделан, работа отложена в следующую сессию по решению заказчика. Здесь записано, что это такое и куда относится, чтобы не разбирать заново.
...
Цвет текста тега (`Н-2а`) при этом требует отдельного разбора: цвета Values пишет модель, и пишет в пути версии 1, то есть под общее объяснение он не подходит. Два кандидата, оба непроверенные: рантайм применяет `textColor` не при всяком `visibility`, либо `migrateConfig` выбрасывает запись цвета, если токен записан без решётки (`normalizeTagToken` возвращает пустую строку, и ключ удаляется). Начинать — с проверки на настоящем пути записи, как с дефектом ссылки.
...
**Н-4. У TagWheel пропали обрамляющие `==` и вместе с ними заливка фона.** Не разбиралось. Заказчик помнит вид `==field1 [field2] field3==`, дававший фон, который и настраивается. Это либо регрессия в отрисовке TagWheel, либо тот же Н-2 (цвет фона задан в новой панели и не доехал). Первым делом — установить, какая из двух причин, и только потом чинить.
