...

{
  const before = {
    transform: {
      inline2note: {
        /*
         * Папка шаблонов названа, и это не украшение фикстуры (У-38): с
         * 2026-09-16 шаблон, не лежащий в назначенной папке, снимается
         * нормализацией (В-127). Без этой строки проверка «имя правила
         * пережило нормализацию» краснела бы на снятом шаблоне, то есть
         * говорила бы о другом предмете.
         */
        templatesFolder: "Templates",
        smartRules: [{
          id: "rule-1",
...

{
  ...
  /*
   * Шаблон назван так же, как его называет список, из которого его выбрали:
   * путём внутри `Templates folder`, а не путём целиком (замечание по S6,
   * 2026-09-09: «показывай только название заметки»). Папка при этом остаётся
   * путём — её он назвать коротко не просил, и короткое имя папки ничего не
   * адресует.
   */
  ...
  /*
   * Положительный контроль к сокращению: имя шаблона — путь **внутри**
   * назначенной папки, а не то, что осталось от последней косой черты. Без
   * него утверждение выше выполнял бы и обход, отрезающий всё до неё, — а
   * тогда два `task.md` в разных подпапках стали бы в сводке неразличимы.
   *
   * **Предмет контроля сменился 2026-09-16 вместе с В-127.** Раньше им был
   * шаблон **вне** папки шаблонов, и он назывался путём целиком; теперь такой
   * шаблон снимается нормализацией, то есть предмета в продукте больше нет, а
   * контроль на несуществующем предмете охраняет пустоту (У-127). Вопрос он
   * задаёт тот же, и подпапка на него отвечает ничуть не слабее.
   */
  const nested = makePanel(baseConfig([{
    id: "r9", enabled: true, name: "Nested",
    targetTemplate: "Templates/sub/task.md",
    conditions: { tags: ["#todo"] },
  }]), { expanded: new Set<string>() });
  assert.equal(all(nested.host, "io-rule__sumpart").map(n => n.textContent)[1],
    "Template: sub/task.md",
    "шаблон из подпапки назван путём внутри папки шаблонов: обрезка по последней косой тут соврала бы");
  /*
