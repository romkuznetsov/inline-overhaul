 ...
 * ничего не сообщает, а шестой род пришлось бы объяснять словами. Дом у списка
 * один — сборщик сводки, — и читается он оттуда: вторая копия разошлась бы
 * молча (У-32).
 */
const CHANGE_MARKS = gen.CHANGE_MARKS;
...
function scanChangelog(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (head) {
      ...
      /*
       * Длины у сводки больше не спрашивается: короткая она или длинная,
       * решают пункты раздела, а равенство им проверяет `summaryMismatch`.
       * Порог в сорок знаков был мерой «сводка не пуста» и стал ложным в тот
       * день, когда у выпуска из двух внутренних правок сводка честно вышла
       * короче (У-246: утверждение о состоянии стареет молча).
       */
      if (marker !== "> [!NOTE]" || !/^>\s+\S/.test(body)) {
...

{
  ...
  /* Бета слабее своего выпуска — это правило `docs/dev/VERSIONING.md`. */
...

{
  ...
  assert.deepEqual(scan.noSummary, [],
    ...
    + " (коллаута)»:\n  " + scan.noSummary.join("\n  "));
  /*
   * **Сводка считается по пунктам своего же раздела** — его слово 2026-09-22:
   * «в changelog мне не нравится как сделан коллаут — куча склеенного текста…
   * суммаризировать инфу (исправлено Х багов, сделано У улучшений интерфейса
   * настроек, добавлены следующие возможности (список с буллитами))».
   *
   * Числа в сводке — утверждение о состоянии раздела, и написанное руками оно
   * стареет молча в день, когда пункт дописали (У-246). Здесь у утверждения и
   * его предмета один дом: сводка выводится из пунктов, и сторож спрашивает
   * равенство.
   */
  const wrongSummary = gen.summaryMismatch(text);
  assert.deepEqual(wrongSummary, [],
...

{
  ...
  /*
   * **Контроль к сводке, и он про числа, а не про форму.** У `9.9.8` сводка
   * стоит на месте и написана словами — то самое «куча склеенного текста», с
   * которого он пришёл; у `9.9.9` её нет вовсе. Названы обязаны быть оба.
   */
  const wrongBad = gen.summaryMismatch(bad);
  assert.ok(wrongBad.indexOf("9.9.9") >= 0, "раздел без сводки не назван: " + wrongBad.join(", "));
  assert.ok(wrongBad.indexOf("9.9.8") >= 0, "сводка словами не названа: " + wrongBad.join(", "));
  /*
   * И вторая половина: сводка **верной формы** с неверным числом обязана быть
   * названа, а верная — нет. Без этой пары запрет проверял бы вёрстку и молчал
   * о том, ради чего заведён.
   */
  const counted = [
    "## 8.8.8",
    "",
    ...
    "> 🐛 **9** bug fixes",
    "",
    "1. 🐛 **Одна починка, а в сводке девять.**",
    "",
    "## 8.8.7",
    "",
    gen.buildSummary("1. 🐛 **Одна починка, и сводка её же.**"),
    "",
    "1. 🐛 **Одна починка, и сводка её же.**",
  ].join("\n");
  const wrongCounted = gen.summaryMismatch(counted);
  assert.deepEqual(wrongCounted, ["8.8.8"],
    "сводка с неверным числом обязана быть названа, а верная — нет: " + wrongCounted.join(", "));
