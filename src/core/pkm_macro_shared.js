...
function getCursorAtTextEnd(finalLine, rules) {
  ...
  return keepOneSpaceBeforeSecondSeparator(line, ch, sep2);
}
 ...
 *
 * Пустой слот под текст сюда не попадает: у него конец равен началу, и двигать
 * курсор из слота наружу нельзя — он бы упёрся в разделитель.
 */
function keepOneSpaceBeforeSecondSeparator(line, ch, sep2) {
  const s = String(line || "");
  const sep = String(sep2 || "");
  if (!sep) return ch;
  const at = s.indexOf(sep, ch);
  if (at === -1) return ch;
