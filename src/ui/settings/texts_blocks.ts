...
export const BLOCK_TEXTS = {
  ...
  "field-editor": {
    ...
    CHILD_DESC: "When the child Field is offered: after a parent Value, always, while you hold Alt, or never",
    CHILD_TIP: "A child Field is a second Field that only makes sense under this one: its Values are the ones marked child in the table below. <b>After parent</b> offers it once the parent has a Value — that is how it has always worked. <b>Show always</b> offers it on any line: tagWheel shows it with no parent picked, and its commands step through every child Value in turn. <b>Show when press Alt</b> keeps it out of tagWheel until you hold <code>Alt</code> on the parent: it appears next to the parent and stays while you hold the key, so you can step onto it and pick a Value — even before the parent has one, and then the Value stands without a parent. Let go and it hides again, and the Value you picked stays on the line. Its commands work as with <b>After parent</b>. <b>Hide</b> keeps it out of tagWheel and out of its commands even when its Values are set up",
