...
async function main(): Promise<void> {
  ...
  await test("перенесены все группы с настройками", () => {
    assert.equal(SCHEMA.length, 40,
      ...
      + "Группа `Smart Enter` заведена 2026-09-13 по его заказу (10.13.88): . Группа `Links in the notes you mention` заведена 2026-09-17 по его заказу Н4 (10.13.184): ссылка на новую заметку уезжает в те заметки, на которые ссылалась строка. Группа `Jump highlight` заведена 2026-09-17 его словом «перенеси все настройки jump-flash в Visual отдельным блоком настроек»: строки не новые — они ушли из группы `Moving cursor inside a note` вместе со своей веткой конфига"
      + "заголовок соседней названа двумя клавишами, и третья в него не "
      + "помещается");
    const bound = SCHEMA.flatMap(g => g.items).filter(isBound);
    assert.equal(bound.length, 143,
