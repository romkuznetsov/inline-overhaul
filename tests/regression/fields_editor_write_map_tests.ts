 ...
 *
 * **Первая карта с 2026-09-06 заморожена, и переснять её больше нечем.** Сама
 * доска снята фазой 6, вместе с ней ушёл и тест, который эту карту печатал
 * (`order_board_write_map_tests.ts`). Файл остаётся и остаётся под контролем
 * версий: он единственная запись о том, куда старая доска умела писать, и
 * читает его именно эта проверка. Тот же уклад, что у
 * `tests/prototype/v1_inventory.tsv` — снимок прошлого, который не правится.
 * Если он однажды разойдётся с новым редактором, разбирать надо новый
 * редактор, а не подгонять снимок.
 *
