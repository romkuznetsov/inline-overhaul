 ...
 * записывается и этим разборщиком не читается вовсе. То есть у одного файла не
 * один читатель, а несколько, и читают они его по-разному. Список того, что
 * меняет ход через диск, выписан ниже утверждениями, а не словами: он и есть
 * задание на перевод движков.
 *
 * **Здесь стояло «блок дат читают своими разборами `navigation_runtime.js` и
 * `status_date.js`», и это было неверно.** Свой разбор был только у навигации,
 * а `status_date.js` берёт правила тем же `parseRulesFromMarkdown` — то есть
 * блока дат не видит. С 2026-09-11 навигация читает настройки напрямую
 * (10.13.52, П-10), и у блока не осталось **ни одного** читателя: он
 * записывается и не читается никем. Ветка конфига, из которой он собирается
 * (`pkm.behavior.dateRules`), пуста у всех, кого можно спросить, — её нет ни в
 * одной фикстуре, включая снимок конфига заказчика, и ни один контрол панели в
 * неё не пишет. Снимается блок вместе с самим документом, шагом четвёртым.
 *
...
for (const name of FIXTURES) {
  ...
  assert.strictEqual(
    ...
    undefined,
    `${name}: и разборщиком TagWheel не читается — с 2026-09-11 не читается вовсе никем`,
  );
...
/*
 * Служебный блок `meta`: в нём нет ничего, кроме служебного, — иначе через
 * него уезжала бы настройка, и «прямое чтение» её потеряло бы.
 *
 * **Отметки времени в нём больше нет** (Д-1, 2026-09-08). Пока она была,
 * содержимое файла всегда было новым, и запись в `onload` меняла файл на
 * каждом запуске Obsidian: синхронизация человека видела правку на ровном
 * месте. Утверждение здесь стояло обратное — «кем и когда собран», — и его
 * пришлось переписать вместе с предметом (У-94).
 */
{
...
/*
 * **Один конфиг — один и тот же файл.** Это и есть половина Д-1: пока в
 * документе стояло время сборки, две сборки одного конфига давали два разных
 * текста, и «не писать без нужды» было невозможно в принципе.
 *
 * Сверяются два прогона подряд, а не одна строка: отметка времени в секундах,
 * и первая версия такой проверки прошла бы просто потому, что уложилась в
 * одну секунду. Поэтому между прогонами стоит настоящее ожидание — оно тут не
 * про скорость, а про то, что предмет измерения существует.
 */
{
  const cfg = internals.migrateConfig(
    JSON.parse(fs.readFileSync(path.join(root, "tests", "fixtures", "config_v1_full.json"), "utf8")),
