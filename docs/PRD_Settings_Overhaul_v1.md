...
**В7. Показывать ли строку дочернего Field в левом списке редактора Fields?** Открыт 2026-08-28, ответа нет.

Ф3 и Ф20 описывают строку дочернего Field (`<name>_sub`) в левом списке. В плагине она не появляется **ни разу**: настоящий `normalizePkmOrder` из `main.js` выбрасывает ключи `_sub` из `left` и из `right`, а левый список строится по ним. То есть либо строку надо показывать вопреки Order, либо Ф3 и Ф20 описывают то, чего нет, и их надо признать излишними.

Вопрос не технический, а о том, что должно быть в панели: дочерний Field настраивается стрелкой `Level` у родителя, и отдельная строка у него может быть лишней сущностью, а может быть единственным местом, где видно, что он вообще есть. От того же условия зависит перетаскивание Block: если строки нет, дочерний Field не перетаскивается отдельно от родителя. **До ответа `listFields` не трогается.**
