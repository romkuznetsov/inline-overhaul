...
async function main() {
  ...
  try {
    ...
    /*
     * И граница: тег без цвета **в вашем тексте** пузыря не получает вовсе —
     * там мы не трогаем ничего. Без этого утверждения правка «рисовать пузырь
     * тегу без цвета» прошла бы и в той форме, где она красит весь текст.
     */
    if (smallZones.some((b) => b.token === "#bare")) {
      bad("тег без цвета между разделителями получил наш пузырь — в вашем"
        + " тексте плагин рисовать не должен");
    }
    await page.evaluate(async () => {
