...
(function rowCountTrustSuite() {
  ...
  /* Ряд бывает выше умолчания (У-133): один ряд в строке 33 точек при
     умолчании 31.5 — это по-прежнему один ряд, и делить его нельзя. */
  assertEq(trusted(1, 33, 31.5), true, "ряд выше умолчания остаётся одним рядом");
  assertEq(trusted(1, 47, 31.5), true, "полтора умолчания — всё ещё один ряд");
  assertEq(trusted(1, 48, 31.5), false, "а полтора с лишним — уже два, и счёт не годится");
