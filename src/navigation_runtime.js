...
/**
 * Сосед, который шагом не разбирается: ссылка.
 *
 * **Ссылка — один знак смысла, а не слово в скобках.** Разорванная пополам,
 * она перестаёт быть ссылкой: `[текст подпись](адрес)` — это уже не разметка,
 * а сломанная строка. Его замечание 2026-09-02 про `[[test1]]` было ровно об
 * этом, и с 2026-09-22 то же правило распространено на `[подпись](адрес)`:
 * шаг рвал её молча, и видно это было первым же нажатием.
 *
 * **Вопрос «это ссылка» задан одному дому.** Wikilink спрашивается у
 * `shared_utils.isWikilinkToken` — там он объявлен один раз, и им же живут
 * движки (У-32). У разметки своего дома пока нет, и её форма описана здесь:
 * квадратные скобки, сразу за ними круглые.
 *
 * **Цена названа вслух:** единица шага — то, что стоит **между пробелами**
 * (исключение № 11, 2026-09-02), поэтому целой остаётся только ссылка без
 * пробела внутри. `[a link](адрес)` — два соседа, и шаг по-прежнему пройдёт
 * между ними. Чинится это не здесь, а сменой самой единицы шага.
 */
const MD_LINK_TOKEN_RE = /^\[[^\]\s]*\]\([^()\s]*\)$/;
function isWholeLinkToken(token) {
  const src = String(token || "");
  if (!src) return false;
  return __sharedUtils.isWikilinkToken(src) || MD_LINK_TOKEN_RE.test(src);
}
 ...
 * выделенному ближе.
 *
 * **Слово в своих скобках целым больше не остаётся** — его замечание
 * 2026-09-22: «при выделении одного слова перемещение работает нормально, но
 * если выделить несколько слов, то перемещение сбоит — перепрыгивает через
 * скобки». Он выписал и то, чего ждёт, тремя нажатиями:
 * `слово2 слово3 (слово1)` → `(слово2 слово3 слово1)` →
 * `(слово1 слово2 слово3)` → `(слово1) слово2 слово3`. То есть скобка — шаг
 * своего права, в обе стороны.
 *
 * **Дело было не в числе выделенных слов**, и это измерено: на одном слове
 * шаг перепрыгивал группу так же (`- текст [[…]] хвост` не в счёт — там
 * ссылка). Его пример описывал форму, а не условие.
 *
 * Здесь стояло правило «края парные — сосед целый», и завёл его **я**, а не
 * он: оно берегло `[[test1]]`. Ссылку теперь бережёт признак ссылки, а
 * скобки вокруг прозы стали обычным краем.
 */
function edgeStep(token, direction) {
  const src = String(token || "");
  if (!src || allEdgeMarks(src)) return src.length;
  if (isWholeLinkToken(src)) return src.length;
