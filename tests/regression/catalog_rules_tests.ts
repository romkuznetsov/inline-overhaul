 ...
 *
 * **Долг закрыт 2026-09-09.** Раздел 5 `docs/dev/OBSIDIAN_CATALOG_RULES.md`
...
const RULES: readonly Rule[] = [
  ...
  },
  {
    ...
    why: "Р6 закрыт 2026-09-11, сорок первое исключение к З3: правило «где "
      + "взять активный редактор» объявлено один раз, в "
      + "`src/core/active_editor.js`, и activeLeaf там последняя попытка — "
      + "после getActiveViewOfType и activeEditor",
  },
  {
    what: "глобальное приложение вместо переданного",
    rx: /\bglobalThis\.app\b/g,
    ceiling: 0,
    control: "const app = globalThis.app;",
    why: "П1 закрыт 2026-09-11: приложение приходит вызовом — сам объект "
      + "или `x.app`. Запасной путь через глобальное приложение снят: ни "
      + "один живой вызов до него не доходил, а тихая работа с чужим "
      + "приложением хуже названной причины",
  },
  {
    ...
    why: "чтение перед записью — законное; каталог требует cachedRead только "
      + "там, где читают для показа. Планка опускалась трижды: с шести до "
      + "пяти 2026-09-11 (навигация перестала читать служебный файл правил), "
      + "с пяти до четырёх 2026-09-13 — файла не читает больше ни один "
      + "движок, включая панель TagWheel (PRD 10.13.52, П-8, шаги первый и "
      + "третий), и с четырёх до трёх 2026-09-18 — снят запасной ход "
      + "«порядок Fields из data.json», который не мог сработать ни при "
      + "каком условии (docs/dev/AUDIT_2026-09-18.md, 4.4 и Р-5)",
  },
];
 ...
 * пофайлово в `docs/dev/OBSIDIAN_CATALOG_RULES.md`, Р7.
 */
...
function main(): void {
   ...
   * печатает сторож, а `docs/dev/OBSIDIAN_CATALOG_RULES.md` на него ссылается.
   */
  let errors = 0;
  let warns = 0;
  let logs = 0;
