...
1. Перед переносом поведение снимается тестом `order_deep_editor_state_tests.js`, расширенным до черновиков, истории, диалога конфликта и удаления Field.
2. Редактор переезжает в `custom/fields_editor.ts` **без изменения логики** (Ф12–Ф16).
3. Тот же тест проходит без правок.
