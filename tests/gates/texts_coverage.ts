...
const NOT_READ: ReadonlyArray<{ match: RegExp; why: string }> = [
  ...
  {
    /*
     * Причина переписана 2026-09-06 и стала уже прежней. Была: «старая доска,
     * новая панель её не рисует». Доски в файле больше нет — снята фазой 6, — и
     * та причина пережила бы свой предмет молча (У-71).
     *
     * Осталось три строки, и ни одна не доходит до человека: `"use strict"` и
     * два диагностических текста, которые кладутся в `__orderDeepEditorStateDiag`
     * и никем не читаются. Проверено снятием этой записи: гейт назвал ровно их
     * и ничего больше.
     */
    match: /fields_editor_legacy\.js$/,
    why: "шов к помощникам состояния: три строки, и все три диагностические — на экран не выходит ни одна",
  },
];
