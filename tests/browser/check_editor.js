...
async function main() {
  ...
  try {
    ...
    if (!plainSmall || !plainBig) {
      ...
      /* Фон — тот, каким тема рисует тег (`--tag-background` страницы), его
         слово цикла 89: «по умолчанию… как в используемой теме». */
      if (String(plainSmall.background) !== "rgba(112, 93, 207, 0.1)") {
        bad("пузырь тега без цвета не взял фон тега темы («" + plainSmall.background
          + "», ждали rgba(112, 93, 207, 0.1))");
      }
      /*
    ...
    /*
     * **Границу он снял сам 2026-09-24:** «все теги, даже которые пользователь
     * не задавал в user-tag-colors, управлялись контролами tag view». Тег без
     * цвета в чужой строке получает наш пузырь — но только там, где тегом его
     * считает Obsidian: в обратных кавычках его нет.
     */
    if (!smallZones.some((b) => b.token === "#bare2")) {
      bad("тег без цвета в строке без разделителей пузыря не получил —"
        + " размер и форму ему по-прежнему рисует тема");
    }
    if (!smallZones.some((b) => b.token === "#bare5")) {
      bad("положительный контроль: тег строки с кодом пузыря не получил —"
        + " отказ в обратных кавычках проверялся бы отказом везде");
    }
    if (smallZones.some((b) => b.token === "#789")) {
      bad("число `#789` в чужой строке получило наш пузырь — для Obsidian это не тег");
    }
    if (!smallZones.some((b) => b.token === "#456")) {
      bad("в строке с одним разделителем число `#456` пузыря не получило —"
        + " строка с одним разделителем перестала считаться строкой плагина");
    }
    if (smallZones.some((b) => b.token === "#code1")) {
      bad("тег в обратных кавычках получил наш пузырь — сканер нарисовал то,"
        + " чего Obsidian тегом не считает");
    }
