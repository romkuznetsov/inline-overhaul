...
- **Гейты включаются по фазе.** `tests/gates/phase.txt` содержит одно число. При `0` отсутствие схемы — это сообщение, начиная с `1` — падение. **Обновить файл при переходе к фазе 1**, иначе гейт продолжит молча пропускать проверки структуры.
