...
- **П-16. Полосы вкладок в декларативном API нет.** Установлено сборкой 2026-08-24: страница (`type: 'page'`) рисуется **строкой перехода**, а не закладкой сверху. Прототип показывает семь закладок; платформа этого не умеет, и первая сборка дала первый экран, на котором нет ничего, кроме семи слов и стрелок. Решение: **одна вкладка раскладывается на первом экране, остальные становятся строками перехода**. Разложена `General` — она про то, что включено и с чего начать, то есть именно то, что человек должен увидеть, открыв настройки. У каждой строки перехода есть описание из вводного коллаута прототипа и, для модулей, состояние `off` справа — чтобы выключенный модуль был виден, не заходя внутрь. Это отступление от прототипа по форме навигации; двухуровневость (Р5) сохраняется, и прототип приводится к платформе, а не наоборот.
...
1. Семь вкладок по 6.1, группы по 6.2.
