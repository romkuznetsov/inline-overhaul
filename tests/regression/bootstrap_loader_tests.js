...
async function run() {
   ...
   * нет.
   *
   * **И это не копия, а два вопроса** — ответ заказчика 2026-09-15 (В-118):
   * «пользователь может вводить как угодно (`- [x]` / `[x]`), а плагин сам
   * нормализует ввод». Слой настроек стоит на пути **текстового поля**
   * (колонка `Prefix`), и терпимость там верна. Сборка строки принимает уже
   * разобранный токен, и терпимость ей не нужна. Сводить тела нельзя;
   * остаётся развести имена (10.13.144, правило 117).
   */
  {
