...
{
  /*
   * **Помощников состояния модель требует вслух** (10.13.166).
   *
   * Их отдаёт тот, кто модель создаёт, и до 2026-09-16 модель на их отсутствие
   * отвечала молча: `deps.deepState || {}`, а дальше в пяти местах стояло
   * `typeof deep.X === "function" ? дом : свой ответ` — пустое дерево значений,
   * своя нормализация и «ничего не записали» у самой записи дерева. Довод был
   * «доска умеет откатиться на заглушку»; заглушку сняли 2026-09-14, а
   * утверждение о состоянии осталось и держало эти ходы (У-64).
   *
   * Что человек получал бы: редактор значений, который **молча не сохраняет**.
   * Пробой показал, что ни один из этих ходов не исполняется, — кроме одного,
   * и тот оказался не запасным, а единственным объявлением правила.
   */
  const model = createFieldsModel;
  const plugin = { getConfig: () => ({}), setConfigPatch: () => {} };
  const args = {
    plugin: plugin as never,
    normalizePkmOrder: internals.normalizePkmOrder as never,
    pkmOrderFields: [],
  ...
  assert.throws(
    () => model({ ...args } as never),
    /order_deep_editor_state unavailable/,
    "модель собралась без помощников состояния и промолчала об этом",
  );
  assert.throws(
    () => model({ ...args, deepState: { normalizeToken: () => "" } } as never),
    /order_deep_editor_state unavailable: normalizeCheckboxInput/,
    "модель собралась с половиной помощников и промолчала об этом",
  );
  /* Контроль: с настоящим модулем сборка проходит — иначе краснело бы всегда. */
  assert.ok(model({ ...args, deepState: deepState as never } as never),
    "положительный контроль: с настоящими помощниками модель собирается");
