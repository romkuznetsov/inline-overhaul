...
function intro(): string {
  return [
    ...
    ">   and Emoji Values and decide how they behave",
...
function firstSteps(): string {
  return [
    ...
    "\t   Emoji Values you set up in `Tags & PKM → Fields`. (*I use `Alt + Arrow down`*)",
    ...
    "   arrow buttons if you prefer, and the line follows. The colors come from the",
...
function fieldsAndValues(): string {
  return [
    ...
    "Value, so it is tags, or wikilinks, or Emoji Values, never a mix.",
    "",
    ...
    "a `#wip`. Tag Values and link Values can do this; an Emoji Field has no list of",
...
function tagWheel(): string {
  return [
    ...
    "* **`Active Field text color`** and **`Inactive Field text color`** are what tells",
    "  the current Field from the rest. Your tag colors are not drawn inside the panel, so",
...
function bars(): string {
  return [
    ...
    "",
    "A Bar is a colored stripe in the margin of a note. It runs down the side of a line",
    ...
    "colors come from the Values of that Field, so a Bar and a bubble can never disagree.",
