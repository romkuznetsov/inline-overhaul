...
async function main(): Promise<void> {
  ...
  await test("перенесены все группы с настройками", () => {
    assert.equal(SCHEMA.length, 35,
      ...
      + "команд из фазы 2, а `Settings backup` заведена в тот же день вместо "
    ...
    assert.equal(bound.length, 117,
      ...
      + "`Highlight the line` добавлен 2026-09-01 (10.13.6): движок обёртку "
  ...
  await test("кнопка действия гаснет на время работы (5.6)", async () => {
    ...
    const saveButton = (): { label: string; disabled: boolean } => {
      const row = rowsOf(pane, "advanced", "Settings backup").find((it: Def) => typeof it.render === "function");
  ...
  await test("подпись id читается в момент открытия подсказки", async () => {
    /*
     * Тот же счёт к `Show setting ids`: тумблер человек щёлкает при открытом
