...
const SECOND_CHUNK: ReadonlyArray<{ file: string; left: number; why: string }> = [
  ...
  {
    /*
     * Имена знаков выбиралки (`В-182`, 2026-09-22). Имя одно на три дела:
     * подпись при наведении, слово поиска и имя команды, которое окно Binder
     * предлагает само («при `→` должно быть `Arrow right`»). Третье решает:
     * имя команды показывает палитра Obsidian, и переводить его нельзя (Я2).
     * Переведённая подпись разошлась бы с именем, которое окно подставит.
     * Число — то, что гейт признаёт прозой; однословные имена он не считает.
     */
    file: "src/ui/settings/custom/char_picker.ts",
    left: 9,
    why: "имена текстовых рожиц: они же имена команд Binder, которые окно предлагает само, а имена команд не переводятся (Я2)",
  },
  {
    /*
     * Имена эмодзи и символов и имена рубрик (его слово 2026-09-23: «столько
     * же, сколько в эмодзи windows 11» и рубрикатор). Файл пишет генератор
     * `tools/build/gen_pick_data.js` из данных Unicode; причина та же, что у
     * рожиц выше, а рубрики — это имена групп Unicode, как в панели эмодзи
     * Windows.
     */
