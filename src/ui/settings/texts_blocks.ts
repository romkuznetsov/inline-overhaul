...
export const BLOCK_TEXTS = {
  ...
  "field-editor": {
    ...
    CHILD_TIP: "A child Field is a second Field that only makes sense under this one: its Values are the ones marked child in the table below. <b>After parent</b> offers it once the parent has a Value — that is how it has always worked — and a press of <code>Alt</code> on the parent opens it before that. <b>Show always</b> offers it on any line: tagWheel shows it with no parent picked, and its commands step through every child Value in turn. <b>Show when press Alt</b> keeps it out of tagWheel until you press <code>Alt</code> on the parent: it appears next to the parent, so you can step onto it and pick a Value — even before the parent has one, and then the Value stands without a parent. Press <code>Alt</code> again and it hides, and the Value you picked stays on the line. <code>Alt</code> together with another key is a hotkey and does not open it. Its commands work as with <b>After parent</b>. <b>Hide</b> keeps it out of tagWheel and out of its commands even when its Values are set up",
    CHILD_ALWAYS: "Show always",
    CHILD_AFTER_PARENT: "After parent",
    CHILD_ALT: "Show when press Alt",
    CHILD_HIDE: "Hide",
    ...
    CHILD_PARENT_TIP: "Shown only with <b>Show always</b>. <b>Leave the line alone</b> writes what you picked and nothing else. <b>Add the parent Value</b> also writes the parent Value that this child Value belongs to, so the pair on the line is one you set up here",
    CHILD_PARENT_KEEP: "Leave the line alone",
    CHILD_PARENT_ADD: "Add the parent Value",
