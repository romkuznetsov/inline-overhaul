...
const tagwheelCore = require(path.join(__dirname, "..", "..", "src", "pkm_v2", "TagWheel", "tagwheel_core.js"));
const tagwheelPanel = require(path.join(__dirname, "..", "..", "src", "pkm_v2", "TagWheel", "tagwheel.js"));
...
function run() {
   ...
   */
  /*
   * **И вопрос здесь «всё ли это значения», а не «есть ли среди них
   * значение».** Первая версия признака спрашивала второе и была **шире
   * сборки**: на `#  :: слово #важное` сборка приклеивает к заголовку всё —
   * `#важное` есть тег внутри слова человека, — а доводка оставляла
   * разделитель. Две половины плагина расходились, и написанное им самим он
   * прочесть не мог (У-157).
   *
   * Поэтому ниже к каждой паре стоит **вторая, обратная**: смесь слова и тега
   * склеивается, как её склеивает сборка. Проверено обеими формами
   * разделителей: его слово 2026-09-15 — совпадающие разделители у
   * большинства, и случай этот основной, а не край.
   */
  ...
  for (const [sep2, name] of [["::", "одинаковые разделители"], ["~~", "разные разделители"]]) {
    ...
    const HEADING_SLOT_CASES = [
      ...
      ["#  :: слово #важное", "# слово #важное", "смесь слова и тега приклеивается целиком"],
      ["#  :: #work слово", "# #work слово", "смесь тега и слова приклеивается целиком"],
    ];
    for (const [input, expected, what] of HEADING_SLOT_CASES) {
      assertEq(
        unified.normalizeStructuredSlots({ rawLine: input, line: input, rules, mode: "minimal" }),
        expected,
        "заголовок и слот текста: " + what + " (" + name + ")"
      );
      /*
       * **И тот же вход спрашивается у сборки — но только при совпадающих
       * разделителях.** Веса в равенстве сторон нет (У-194), поэтому выше
       * стоит ожидание ответа; здесь ловится расхождение двух половин плагина
       * между собой — ровно то, которым первая версия признака была плоха.
       *
       * При разных разделителях сравнивать нечего: строка входа написана с
       * одним знаком, и сборка правый Block отделяет **вторым** — ответы
       * разные по построению, а не по расхождению. Совпадающие разделители —
       * его случай и есть (его слово 2026-09-15), и именно там текст и правый
       * Block делит один и тот же знак.
       */
      if (sep2 === "::") {
        assertEq(
          linePipelineForSlots.buildFromSegments(linePipelineForSlots.splitSegments(input, rules), rules),
          expected,
          "заголовок и слот текста, сборка отвечает так же: " + what
        );
      }
    }
