...
- **Ф17.** Доступность: у списка Fields есть клавиатурный путь. В каждой строке справа две кнопки, `↑` и `↓`; они переставляют Field внутри Block, а на краю Block переводят его через линию — вниз с низа `Left Block` в начало `Right Block`, вверх с верха `Right Block` в конец `Left Block`. Другого способа сменить сторону с клавиатуры нет, поэтому это поведение обязательное, а не удобство.
...
- **Ф20.** У дочернего Field своих стрелок нет: он ходит за родителем (Ф3).
...
2. Имена команд по 7.2 (ID пока не меняются).
...
5. `manifest.json`: заполнить `authorUrl`; `fundingUrl` **предусмотреть полем** со значением-плейсхолдером (решение заказчика от 2026-08-24). **Перед подачей в community** плейсхолдер обязан быть заменён настоящей ссылкой или поле удалено: Obsidian показывает `fundingUrl` кнопкой в карточке плагина, и нерабочая ссылка — замечание на review. Проверка входит в чек-лист фазы 6.
...
Открытых вопросов нет: все шесть закрыты заказчиком 2026-08-24. Новый вопрос дописывается сюда и задаётся до начала соответствующей фазы, а не решается самостоятельно (раздел 0).
