...

{
  ...
  /*
   * Положительный контроль к сокращению: шаблон **вне** назначенной папки
   * остаётся со своим путём целиком. Без него утверждение выше выполнял бы и
   * обход, отрезающий всё до последней косой черты, — а тогда два `task.md` в
   * разных подпапках стали бы в сводке неразличимы.
   */
  const outside = makePanel(baseConfig([{
    id: "r9", enabled: true, name: "Elsewhere",
    targetTemplate: "Other/task.md",
    conditions: { tags: ["#todo"] },
  }]), { expanded: new Set<string>() });
  assert.equal(all(outside.host, "io-rule__sumpart").map(n => n.textContent)[1],
    "Template: Other/task.md",
    "шаблон не из папки шаблонов назван путём целиком: короткое имя тут врало бы");
  /*
