...
function controlFor(it: SettingDef, w: Wiring): SettingControl | undefined {
  ...
  if (it.kind === "slider") {
     ...
     *
     * Пробел здесь **неразрывный**. Обычный переносится, и на трёхзначном
     * значении знак единицы уезжал под число: «в настройках tags-text-size при
     * установке 100% и дальше символ `%` переносится на следующую строку»
     * (заказчик, 2026-09-07). Ширину этой ячейки решает платформа, и от
     * ширины строки она меняется — поэтому перенос запрещается в самом
     * тексте, а не правилом стилей: правило пришлось бы искать по чужому
     * классу и держать в двух местах (У-32).
     */
    if (it.unit) {
      const unit = it.unit;
