...
**Н-3. Field типа `link` и `element` уходят в Right Block и встают крайними справа.** К слою настроек отношения не имеет и нами не внесено: это тот же узел «списки определений против Blocks», о который споткнулось предусловие. Определения ссылок и элементов лежат в `rightMode.fields` независимо от того, в каком Block Field стоит, а сборка `techOrder` в `applyOrderToRules` строит слоты **только по `order.left`** — Field, которого там нет, попадает в общий хвост и печатается последним справа. Настройка `Left Block` при этом честно записана в конфиг: расходится не панель с конфигом, а конфиг с движком.
