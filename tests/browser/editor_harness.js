...
const EDITOR_INJECTIONS = {
  ...
  /*
   * Цель ссылки, показанной как написано, обратно остаётся без цвета (`З-37`):
   * то состояние, в котором два контрола `Link view` не делали ничего.
   * Ломается **правило**, а не скорость (правило 144): пометки не ставятся.
   */
  "link-written-not-painted": {
    file: "src/ui/editor/decorations.js",
    find: "          const parts = writtenLinkParts(token, from, to);",
    replace: "          const parts = null;",
  },
  /*
   * И вторая беда того же места: цель и скобки красятся **одним** цветом.
   ...
   * перестаёт находить свой узел. Ровно то состояние, в котором настройка
   * доходит до слоя и не делает ничего (У-16, У-67).
   */
  "link-written-loses-cascade": {
    file: "styles.css",
    find: ".io-linkwritten__target { color: var(--io-link-target, inherit); }",
...
function injectionSpec(name, list) {
  ...
  const spec = from[name];
  /*
   * **Имя файла у подмены выбирает механизм, и ошибка в нём не видна ничем.**
   * Лист стилей подменяется на странице и узнаётся ровно по `styles.css`;
   * запись с `src/styles.css` уходила бы в сборщик, где `.css` не грузится
   * вовсе, — то есть подмена **не применялась**, а прогон печатал «проверка
   * осталась зелёной» и обвинял проверку. Куплено этим: правило `З-37` едва
   * не было снято как мёртвое (У-151, правило 73).
   *
   * Путь проверяется по расширению, а не по списку имён: файл стилей у нас
   * один, и станет их два — это скажет сама проверка.
   */
  const file = String(spec && spec.file || "");
  if (/\.css$/.test(file) && file !== "styles.css") {
    throw new Error("подмена " + name + " указывает лист стилей как «" + file
      + "»: страница знает его под именем «styles.css», и в таком виде подмена"
      + " не применилась бы вовсе");
  }
  if (!file) throw new Error("у подмены " + name + " не назван файл");
  return spec;
}
...
const PAGE_CSS = [
  ...
  /*
   * ПОДДЕЛКА OBSIDIAN: **свой цвет** у текста ссылки. Без него правило
   * «наш цвет цели выигрывает» проверялось бы отсутствием предмета (У-88):
   * выиграть не у кого, и зелёное значило бы «красить было нечем».
   * Значение то, которым `app.css` 1.13.7 красит `.cm-hmd-internal-link` —
