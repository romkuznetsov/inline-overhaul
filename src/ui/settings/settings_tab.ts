...
/**
 * Где лежит открытая вкладка панели (`Р-13`, его слово «вернуть память»,
 * В-143). Путь объявлен здесь один раз: его спрашивают и чтение при открытии,
 * и запись при переходе (У-32).
 *
 * **Ключ в конфиге был и до этой правки** — его нормализуют и переносят
 * миграцией, — а читать и писать перестали вместе со старой панелью
 * 2026-08-29. То есть это не новая настройка, а возврат пропавшего: снятие
 * ключа, назначенное PRD (строка 8009), отменено его словом.
 */
...
export class SettingsPane {
  ...
  /**
   * Открытая вкладка. **Запоминается между запусками** — его слово «вернуть
   * память» (В-143, `Р-13`): панель открывается там, где её закрыли.
   *
   * В стек отмены запись не попадает (`undoable: false`): «отменить» после
   * перехода по вкладкам обязано отменять правку человека, а не переход.
   * Прежняя запись здесь говорила «в data.json не пишется» — так было с
   * 2026-08-29, когда чтение и запись ушли вместе со старой панелью, а сам
   * ключ в конфиге остался.
   */
  ...
  constructor(deps: TabDeps) {
    ...
    /*
     * Запомненная вкладка (`Р-13`). Спрашивается **у самой панели**, а не у
     * списка в нормализации конфига: тот список объявлен под старую панель и
     * называет `hotkeys` там, где у новой стоит `keyboard`, — и до файла
     * версии 2 он не доходит вовсе (измерено, а не прочитано). Единственный,
     * кто знает, какие вкладки есть сегодня, — схема, выведенная из прототипа.
     *
     * Вкладки без групп сюда не попадают по той же причине, по какой их нет в
     * полосе: пустых страниц мы не рисуем.
     */
  ...
  /**
   * Записать открытую вкладку (`Р-13`).
   *
   * **Тем же швом, каким панель пишет всё остальное** — второй путь записи
   * разошёлся бы с первым молча и миновал бы миграцию (CS10, У-32).
   *
   * **`undoable: false` — оговорка, названная до правки:** переход по вкладкам
   * не ступень отмены. Иначе `Ctrl+Z` в настройках возвращал бы вкладку вместо
   * того, чтобы отменить правку человека.
   *
   * Отказ записи громкий: сломалось невидимое, и человек придёт со словами
   * «перестало запоминаться», а журнал — единственный способ узнать, почему.
   */
...
/** Что в группе отличается от значения по умолчанию. */
   ...
   *
   * **Почему не своей строкой под заголовком, как было до 2026-09-01.** Строку,
   * у которой есть только `desc`, платформа **не рисует вовсе** (`app.js`,
   * `Z2`: нужно `name`, `render`, `control` или `action`). Из-за этого вводных
   * фраз в панели не было никогда, а вместе с ними до окна не доезжало тело
   * подсказки группы, которое ехало той же строкой, — отсюда четыре захода
   * «при нажатии на "?" ничего не происходит» (B7, C18, C41, C42).
   *
  ...
  private groupTipButtonFor(
    ...
    return (btn: ExtraButtonComponent) => {
      ...
      return btn.setTooltip(label).onClick(() => {
         ...
         * определений: тумблер `Show option IDs` человек может щёлкнуть при
