...
async function main(): Promise<void> {
  ...
  await test("перенесены все группы с настройками", () => {
    ...
    assert.equal(bound.length, 152,
      ...
      + "назвал сам — `Only custom name`, `Default name`, `Custom+Default name`. И тумблер `Smart paste` добавлен 2026-09-21 по его пункту 9 (`З-31` и `З-32`): вставленный нумерованный список считается с единицы, а знак вставленного пункта в строке, у которой знак уже есть, снимается");
  });
