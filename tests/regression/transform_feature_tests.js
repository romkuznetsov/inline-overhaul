...
/*
 * Три правила, которые до 2026-09-11 держались ни на чём (заход 7 ревизии).
 *
 * Подмена каждого из них в продукте не роняла **ни одной** из 67 проверок:
 *   1. разделители меняются местами;
 *   2. явный тип Field перестаёт учитываться;
 *   3. тип из порядка перестаёт учитываться.
 *
 * Причина у всех одна и та же, и она не в коде, а в примере (У-47): в каждой
 * фикстуре `separator1` равнялся `separator2`, а `field.type` совпадал с тем,
 * что говорит `types` порядка. Когда две стороны согласны, нельзя увидеть,
 * какую из них читают, — и подмена проходит незамеченной.
 *
 * Поэтому здесь они **разные нарочно**:
 *   - разделители `::` и `//` — каждый обязан встать в свой слот;
 *   - у `link` явный тип `wikilink` **спорит** с типом из порядка (`element`),
 *     и выиграть обязан явный;
 *   - у `when` явного типа нет вовсе — работать обязан тип из порядка.
 *
 * Ожидание выписано целой строкой, отдельно от того, из чего она считается
 * (У-5): подмена любого из трёх правил меняет её вид, а не только число.
 */
(function testPreviewReadsBothSidesWhenTheyDisagree() {
  const cfg = {
    transform: { inline2note: { yamlNoteFormat: "clean" } },
    pkm: {
      lineFormat: { separator1: "::", separator2: "//" },
      fields: {
        order: {
          ...
          active: {},
          enabled: {},
          /* про `link` порядок говорит «элемент», а сам Field — «ссылка» */
          types: { kind: "tag", link: "element", when: "element" },
          propertiesByField: {},
        },
        tags: { fields: [{ id: "kind", prefix: "#", values: [{ token: "todo" }] }] },
        links: {
          fields: [
            { id: "link", type: "wikilink", values: [{ token: "ProjectA" }] },
            /*
             * А у этого своего типа нет, и догадка по виду назвала бы его
             * тегом: есть решётка и есть значение. Единственный, кто говорит
             * «элемент», — порядок, и слушать надо его.
             */
            { id: "when", prefix: "#", values: [{ token: "later" }] },
          ],
        },
      },
    },
  ...
  assertEq(
    transform.buildPreviewBaseLine(cfg),
    "- [ ] #todo :: buy milk and bread on the way home after work today // [[ProjectA]] #value",
    "разделители встают каждый в свой слот, явный тип Field выигрывает у типа из"
    + " порядка, а Field без своего типа берёт тип из порядка",
  );
})();
...
/*
 * `Line above is a heading` (10.13.9, замечание заказчика 1.6.4.1).
 *
