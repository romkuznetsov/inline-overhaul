...
- **Н15.** В таблице Values редактора Fields пара «фон значения» и «цвет текста» проверяется на контраст по WCAG 2.1 (отношение относительных яркостей, порог **4.5:1**).
- **Н16.** При значении ниже порога в строке появляется тихий значок с подсказкой: `This Value may be hard to read: contrast 2.8:1, aim for 4.5:1`. Точное число показывается — оно объясняет, насколько далеко до нормы.
...
| Блок | Чего ждёт |
|---|---|
| Свойства заметки (10.9) | Слоя данных Fields: блок пишет `property`, `cardinality` и `valueRule` **в Field**, а не в настройку. Приезжает с редактором в 3b |
| Smart Rules (10.8) | Того же: выбор значения в условии — это выбор Value существующего Field |
| Предпросмотр строки (10.3) | Того же: он показывает Fields, а не значения одного Field |
