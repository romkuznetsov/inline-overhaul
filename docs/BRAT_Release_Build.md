...
**Ветка: пример выше предполагает, что работа идёт в `main`, а сейчас это не
так.** Работа лежит в `docs/settings-overhaul-prd`, и `origin/main` от неё
отстаёт — на 2026-09-10 на 111 коммитов, причём своего в `main` нет ничего
...
**Тег важнее ветки:** `release.yml` ловит `push` тега вида `[0-9]*` и собирает
из того коммита, на который тег указывает; в какой он ветке — workflow всё
равно, и вложения релиза от этого не зависят. Ветка выкладывается затем, чтобы
на GitHub было видно, из чего собрано. **Сдвигать ли `main` на эту ветку —
решение заказчика** (В-96): `main` у него ветка по умолчанию, и на неё смотрит
всякий, кто открывает ссылку на репозиторий.
...
Точка входа сборки — `main.js`. Реестра плагин-локальных модулей и моста
`vault_module_bridge.js` больше нет: **сняты 2026-09-07**, вместе с `build/release_entry.js`.
Все свои модули приезжают литеральным `require` и попадают в бандл графом
сборки, а не поиском по пути внутри vault. Горячая перезагрузка BRAT отдаёт то,
что собрано: устаревшему кешу взяться негде.
