 ...
 *
 *     node tools/build/gen_schema.js
 *
...
for (const g of groups) {
  ...
  for (const it of items) {
    ...
    /*
     * **Проверка на симптом: запись без вида — не запись** (У-96, правило 27).
     * Свой разборщик врёт молча, и врал: `{}` внутри комментария прототипа
     * (`Decoration.replace({})` в объяснении к `З-37`) разбор посчитал
     * отдельной записью, и в схему уехало пустое `{}`. Все семь шагов при
     * этом были зелёные — пустая запись не нарушает ни одного правила, она
     * просто есть. Признак у беды один и он здесь: у записи нет `kind`.
     */
    if (!kind) {
      throw new Error("группа " + id + ": запись без `kind` — «"
        + it.replace(/\s+/g, " ").slice(0, 80)
        + "». Скорее всего, скобки внутри комментария прототипа разбор принял за запись");
    }
