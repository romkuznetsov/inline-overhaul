...
async function run() {
  ...
  /*
   * **Шаг по циклу: тоже два вопроса под одним именем, и тоже разведены**
   * (10.13.154, 2026-09-15). `status_date.js` берёт **значения поля** и
   * булево «вперёд», `status_tags.js` — **готовые токены** и слово. Пока имя
   * было одно, мера копий считала их одним предметом, а расхождение — 24 пары
   * из 156 — читалось как долг, хотя ни одна из этих пар в продукте не
   * достижима.
   *
   * Ни то, ни другое объявление наружу не отдано, и позвать их ответом
   * неоткуда: поэтому здесь проверяется **форма** — каждое имя стоит в своём
   * файле и ни одно не вернулось в чужой. Это ровно тот сторож, который по
   * У-195 ловит возврат копии, и единственный возможный для непубличного тела.
   */
  assertTrue(/function nextCycleTokenFromValues\(/.test(codeOnly(statusDateSrc)),
    "шаг по циклу у элемента назван своим вопросом: вход — значения поля");
  assertFalse(/function nextCycleTokenByDirection\(/.test(codeOnly(statusDateSrc)),
    "прежнее общее имя не вернулось в status_date.js: у двух вопросов не бывает одного имени");
  assertTrue(/function nextCycleTokenByDirection\(/.test(codeOnly(statusTagsSrc)),
    "шаг по циклу у тегов остался при своём имени: вход — готовые токены");
  assertFalse(/function nextCycleTokenFromValues\(/.test(codeOnly(statusTagsSrc)),
    "имя дорожки значений не завелось в status_tags.js");

  /*
   * **Два вопроса, а не копия, и теперь у них разные имена** (В-118, ответ
   * заказчика 2026-09-15: «пользователь может вводить как угодно — `- [x]` или
   * `[x]`, — а плагин сам нормализует ввод»; правило 117, разбор 10.13.153).
   *
   * Одно имя носили два разных вопроса, и отвечали они по-разному: расхождение
   * измерено 2026-09-15 — восемь входов из 31, и шесть из восьми были началами
   * строк его собственных заметок. Отсюда и разведение:
   *
   *   - `normalizeCheckboxInput` в `order_deep_editor_state.js` стоит на пути
   *     **текстового поля панели** и принимает то, что напечатал человек:
   *     знак списка снимается;
   *   - `normalizeCheckboxToken` в `pkm_line_finalize_unified.js` принимает
   *     **уже разобранный токен**, и терпимость ему не нужна.
   *
   * Закрепляется здесь не равенство сторон, а **ответ каждой** (У-194): на
   * разобранном токене они совпадают, на вводе человека — нет, и это верно.
   * Форму — что прежнее имя в слое настроек не завелось обратно — стережёт
   * запрет ниже.
   */
  {
    const deepState = require(path.join(__dirname, "..", "..", "src", "core", "order_deep_editor_state.js"));
    assertTrue(typeof deepState.normalizeCheckboxInput === "function",
      "нормализатор ввода панели назван своим вопросом");
    assertTrue(typeof deepState.normalizeCheckboxToken === "undefined",
      "прежнее имя в слое настроек не вернулось: у двух вопросов не бывает одного имени");
    assertEq(deepState.normalizeCheckboxInput("- [x]"), "[x]",
      "ввод человека: знак списка снимается перед разбором чекбокса");
    assertEq(lineFinalizerUnified.normalizeCheckboxToken("- [x]"), "",
      "разобранный токен: знак списка не снимается — вход сюда приезжает уже разобранным");
    assertEq(deepState.normalizeCheckboxInput("[x]"), "[x]",
      "ввод человека: разобранный токен проходит как есть");
    assertEq(lineFinalizerUnified.normalizeCheckboxToken("[x]"), "[x]",
      "разобранный токен: проходит как есть");
  }
   ...
   */
  assertTrue(/cursorOutsidePanelStrip\(text, plan,\s*visibleChToTextCh\(text, plan \? plan\.hidden : \[\], getControlCursorCh\(state, control\)\),\s*Boolean\(state\.custom\)\)/.test(tagwheelSrc), "tagwheel updates cursor using cursorPolicy in active control mode");
  ...
  assertTrue(/function cursorOutsidePanelStrip\(text, plan, ch, forward\)/.test(tagwheelSrc), "tagwheel keeps the caret out of its own highlighted strip");
   ...
   * сведены. Остальные названы в очереди `docs/dev/REMAINING_WORK.md`: молчаливый
   ...
   * снят (`docs/dev/AUDIT_2026-09-18.md`, Р-5). Дом следа на месте, и проверяется
   ...
   * 2759 вызовов, заходов ноль (`docs/dev/AUDIT_2026-09-18.md`, 4.4 и Р-5).
   *
  ...
  /* Метка объявлена полем правой стороны — так её кладёт продукт; ключ
     `dates.markers`, который здесь стоял, не пишет никто (10.13.158). */
  const cursorRules = {
    io: { separator1: "||", separator2: "||" },
   ...
   */
  {
    ...
    /*
     * **Положительный контроль стоит на строке-примере, а не на счёте в
     * продукте** (У-127). Здесь стояло «узкого написания в исходниках больше
     * тридцати», и число это **убывает от хорошей работы**: каждое место,
     * переехавшее в общий дом, уносит с собой одно узкое написание. Контроль
     * умирал бы вместе с долгом — 2026-09-17 он и покраснел, когда разбор
     * начала строки в Transform стал спрашивать `lineStartOf`, и узких
     * написаний стало ровно тридцать.
     *
     * Считалка проверяется на образце, ответ на котором известен, и в обе
     * стороны: узкое она находит, широкое — тоже, а ссылку не путает ни с
     * тем, ни с другим.
     */
    const SAMPLE = "/^" + BS + "[[^" + BS + "]]" + BS + "]$/ и /^" + BS + "[[^" + BS + "]]+" + BS + "]$/ и [[note]]";
    assertEq(countOutsideWikilink(SAMPLE, NARROW), 1,
