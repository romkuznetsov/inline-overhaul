 ...
 *
 * **Выравнивать это нельзя, и причина измерена, а не выведена.** Пробой со
 * счётчиком 2026-09-19: обычная запись контрола панели даёт **ноль** вызовов
 * `refreshOpenEditors`, патч своего блока — один. То есть у настройки, которая
 * живёт в декорации, а не в переменной CSS, единственный путь до заметки —
 * ближайшее обновление редактора. Ответ «всегда» и есть этот путь, и охрана по
 * флагам его сузила бы.
 *
 * **Три флага — не то же самое.** `selectionSet` у платформы значит «в
 * транзакции была селекция», а не «она изменилась»
 * (`get selectionSet() { return this.transactions.some(tr => tr.selection) }`,
 * `@codemirror/view`), поэтому толчок из `refreshOpenEditors` их будит. А вот
 * транзакцию из одних эффектов — `reconfigure` компартментов, которым тот же
 * толчок начинается, — не будит ни один из трёх.
 *
 * **Подпись настроек у подложки — четвёртый ответ, и он самый узкий.** Слой
 * `layer(...)` перерисовывается дорого (прямоугольники считаются заново), и
 * ему важно не «случилось ли что-то», а «изменилось ли то, из чего он
 * считает». Разбор — у самой `blockFillLayerNeedsRedraw`.
 *
 * **Цена «всегда» названа числом:** один вызов сборки тегов — 0,45–0,56 мс и от
 * размера документа не зависит (считается окно отрисовки), полосы — 0,08–0,10
 * мс. При наборе текста это около 5 мс в секунду.
 *
