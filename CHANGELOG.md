...
2. 🐛 **A two-digit year is a year again.** A Field whose `Value format` is `yy-mm-dd` used to write the letters `yy` into the line, and then failed to recognise its own value; `YY` is now a token like `YYYY`, `MM` and `DD`, and it writes the last two digits.
