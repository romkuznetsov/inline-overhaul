...
/**
 * Языки, которые плагин кладёт в папку сам.
 *
 * **Список пуст, и это решение заказчика** (2026-09-06, 10.13.48): «пока
 * должен остаться только `default.js` = en. Других языков пока не будет». В
 * папке лежит ровно один файл — тот, который ведёт плагин; всё остальное
 * приносит человек, и регистрировать это негде: список языков собирается из
 * папки.
 *
 ...
 * — но **только пока в них не тронуто ни строки**.
 *
 * Класть их заново он больше не будет (`SHIPPED_LANGS` пуст), а этот список
 * жив ради тех, у кого они уже лежат от прошлых сборок: `en.js` до
 * `0.1.0-beta.4`, `ru.js` до неё же. Не трогать нетронутый значило бы
 * оставить его замороженным, а переписать с правкой человека — унести его
