...
async function main(): Promise<void> {
  ...
  await test("перенесены все группы с настройками", () => {
    ...
    assert.equal(bound.length, 160,
      "настроек, привязанных к путям конфига. Строка `Jump over neighbor trees` добавлена 2026-09-25 его заказом (PRD 10.13.275). Третья строка цвета гиперссылки — адрес — добавлена 2026-09-22 по его замечанию к тесту 2 («сделай отдельный контрол на него»): до неё адрес красили скобки. Две строки цвета гиперссылки добавлены 2026-09-22 по его замечанию к тесту 4 («я хотел, чтобы hyperlinks управлялись отдельными контролами»): прежняя пара осталась за wikilink. Кегль Block разведён на две строки 2026-09-19 по его слову (`Text size of the Left Block` и `Text size of the Right Block`), прежняя одна снята. Строка `Chosen Value text color` добавлена 2026-09-17 по его заказу: у неактивного Field имя поля и выбранное значение рисовались одним цветом. Тумблер `Floating button` снят "
      ...
      + "(З-7, 10.13.59): тумблер заливки Left и Right Block, её цвет и "
      + "густота. "
      + "И две ещё — 2026-09-09, по его замечанию по S7: на сколько "
      + "подложка выходит за написанное — вверх и вниз в точках, в стороны "
      + "в долях расстояния до разделителя. Без них блок из одного "
      + "тега подложки не показывал вовсе: у пузыря свой цвет. И три "
      + "строки добавлены 2026-09-12 по его решению от 2026-09-11 "
      + "(10.13.76): на каком Field открывается TagWheel и, при выборе "
      + "`A Field you choose`, какое поле ведущее в каждом Block. И строка "
      + "`Values in the other Block` добавлена 2026-09-13 по его заказу "
      + "(10.13.87): полоса панели вставала на место своего Block, а "
      + "противоположный уходил из строки на всё время выбора. И две строки "
      + "`Smart Enter` добавлены 2026-09-13 по его заказу (10.13.88): "
      + "тумблер клавиши и знак списка на новой строке. Третья строка той же "
      ...
      + "Н5 (10.13.186): тумблер, цвет, размер, время жизни круга, задержка между "
      + "прыжками (его третий контрол, В-136) и тумблер для шагов внутри строки. "
      + "И строка `Stripe direction` добавлена 2026-09-19 по его заказу З-12: «при "
      + "left — полоска возникает только в left block, при right — только в right "
      + "block», умолчание `both`. И тумблер `Autosave` добавлен 2026-09-19 по его "
      + "заказу З-11 и его же ответу на В-147: копия настроек снимается при открытии "
      + "плагина, если файл разошёлся с последней автокопией; умолчание — выключено, "
      + "потому что заметки в vault человека плагин без спроса не пишет. И два "
      + "тумблера `Link view` добавлены 2026-09-20 по его слову: предпросмотр по "
      + "наведению и перетаскивание у значения-ссылки, показанного своим текстом. "
      + "Обе повадки принадлежат ссылке Obsidian, и у нашего узла их нет, пока "
      ...
      + "назвал сам — `Only custom name`, `Default name`, `Custom+Default name`. И тумблер `Smart paste` добавлен 2026-09-21 по его пункту 9 (`З-31` и `З-32`): вставленный нумерованный список считается с единицы, а знак вставленного пункта в строке, у которой знак уже есть, снимается. И два цвета `Link view` добавлены 2026-09-21 по его заказу `З-37` (уточнён `В-174`, цена скобок названа и принята `В-176`): цель ссылки, показанной как написано, и сами скобки красятся врозь. И два тумблера `Inline to note` добавлены 2026-09-25 по его ответу `В-224` (PRD 10.13.272): `Link to Navigator` и `Keep sub-fields`");
  });
  ...
  await test("подсказка не раскрывается в узел, который стили прячут", () => {
    ...
    /*
     * Хозяин подсказки — в двух формах, и вторая куплена этой же проверкой.
     *
     * Сперва обход знал одну: узел, созданный литералом прямо на месте вызова
     * (`host: el(..., "div", "io-tiphost")`). 2026-09-21 разделы правой
     * колонки Fields стали сворачиваться (`З-34`), и хозяин переехал в
     * переменную — тот же узел, то же имя класса, но обход перестал его
     * видеть вовсе. Покраснел при этом **положительный контроль**, а не
     * запрет: запрет был бы зелёным именно потому, что искать стало нечего
     * (У-94). Теперь обход берёт и переменную: имя переменной сопоставляется
     * с классом, которым её создали.
     */
    ...
    for (const m of blob.matchAll(/const\s+([A-Za-z0-9_]+)\s*=\s*el\([^,]+,\s*"[a-z]+",\s*"(io-[A-Za-z0-9_-]+)"\)/g)) {
      byName.set(String(m[1]), String(m[2]));
    }
    for (const m of blob.matchAll(/host:\s*([A-Za-z0-9_]+)\s*,/g)) {
      const cls = byName.get(String(m[1]));
      if (cls) hosts.add(cls);
    }
    assert.ok(hosts.size > 0,
      "ни одного узла-хозяина подсказки не найдено — проверка ищет пустоту");
    /*
     * Контроль на вторую форму отдельно: без него обход мог бы снова видеть
     * одни литералы, и «пересечение пусто» значило бы «переменных не нашли»
     * (У-142 — контроль на каждый шаг обхода, а не на его вывод).
     */
    assert.ok(Array.from(byName.values()).some(cls => hosts.has(cls)),
      "обход не нашёл ни одного хозяина подсказки, названного переменной");
