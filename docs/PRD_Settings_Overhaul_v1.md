...
| Предпросмотр строки (10.3) | Того же: он показывает Fields, а не значения одного Field. **Хвост П11 закрыт 2026-08-28**: предпросмотры читают настоящие Fields; сам `renderLinePreview` — следующий блок 3c |
