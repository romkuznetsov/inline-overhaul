...
- **Н24.** **Предложить можно только Field из того же списка определений.** Определения тегов лежат в `pkm.behavior.leftMode.fields`, ссылок и элементов — в `rightMode.fields` (`ensureBehaviorModesFromOrder` в `main.js` раскладывает их по типу, а не по Block). `reconcileModeDependencies` в `pkm_rules_runtime_helpers.js` ищет `dependsOn` **внутри своего списка** и, не найдя, ставит `enabled = false`: Field молча замолчал бы. Поэтому тег ждёт тега, а ссылка и элемент — ссылку или элемент. Это сказано в подсказке строки.
...
- **Н29.** **Побочное следствие, которое надо знать.** Ключ `dependsOn` для рантайма означает ещё и «дочерний Field»: `enforceDependentAdjacencyForStatusLine` ставит токен зависимого Field сразу за токеном того, кого он ждёт, а при `Child tag format` = `combined` они сливаются в один тег `#parent/child` (`buildCombinedSelectionSet`). Для Fields правого списка — ссылок и элементов — это не проявляется: обе функции смотрят только левый список. Для тегов это видно, и в подсказке сказано, что два Field держатся в строке рядом. Развести «предусловие» и «дочерний Field» можно только правкой движка, и это отдельная работа за границами фазы 3b.
...
- **ждать можно только «своего»**: тег — тега, ссылка и элемент — ссылку или элемент. Определения лежат в двух списках, и `reconcileModeDependencies` выключает Field, чей `dependsOn` указывает в чужой список (Н24);
- **кольцо запрещено**: обход зависимых Fields в рантайме рекурсивен и без списка пройденных, кольцо повесило бы Obsidian (Н25);
- **`dependsOn` для движка означает ещё и «дочерний Field»**: у тегов зависимый Field встаёт в строке вплотную к тому, кого ждёт, а при `Child tag format` = `combined` они сливаются в `#parent/child` (Н29). У ссылок и элементов этого не происходит. Развести два смысла можно только правкой движка — отдельной работой.
