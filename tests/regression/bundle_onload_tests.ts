 ...
 * корня репозитория, и все его `require("./core/...")` там разрешаются. У
...
async function run(): Promise<void> {
  ...
  /*
   * **Служебного файла правил сборка не пишет** (PRD 10.13.52, П-8, шаг
   * четвёртый). Здесь стояло обратное требование — «файл записан, и блоков в
   * нём десять», — потому что файл был единственным каналом между настройками
   * и движками `pkm_v2/**`. Канала больше нет: правила приезжают ключом
   * `Rules data`, и запись файла из сборки означала бы, что снятие не доехало
   * до человека (У-89: набор гоняет исходники, а у человека стоит сборка).
   *
   * Спрашивается **весь список записанного**, а не один путь: имя файла могло
   * бы и смениться.
   */
  const writtenPaths = Array.from(app.written.keys()).map((p: Any) => String(p));
  const rulesLike = writtenPaths.filter((p) => /generated_rules|RULES_TagWheel/i.test(p));
  assert.deepStrictEqual(rulesLike, [], "сборка записала служебный файл правил: " + rulesLike.join(", "));
  ok(`сборка ничего не пишет в vault сверх своего конфига (записей: ${writtenPaths.length})`);
