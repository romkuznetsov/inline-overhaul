...
/*
 * Замечание заказчика 2026-09-12: «в io-fields-list неудобно работает
 * drag-n-drop — при переносе fields в left block не могу их перетащить на
 * верхнее значение: при наведении выше первого поля выделяется сразу вся
 * таблица, и при отжатии поле перемещается в конец этого block».
 *
 * Так и было. Выше первой строки лежит только подпись стороны, своего броска у
 * неё не было, и событие доставалось стороне целиком — а сторона значит «в
 * конец». Места, которое значит «в начало», не было вовсе: стрелка вверх у
 * первой строки уводит Field на другую сторону, а не поднимает его.
 *
 * Контроль стоит первой половиной самой проверки: сначала Field переносится в
 * конец стороны, и только потом — на подпись. Без него утверждение «Due первый»
 * было бы зелёным и в фикстуре, где он первый и так.
 *
 * Мутация: вернуть в обработчике подписи бросок без `beforeKey` — и проверка
 * краснеет.
 */
{
  const v = makeView();
  const sides = all(v.host, "io-side");
  const due = rowsOf(v.host).find(r => nameIn(r) === "Due") as StubNode;
  dragToSide(due, sides[0] as StubNode);
  assert.deepEqual(layout(v.host), { left: ["Status", "Due"], right: [] },
    "контроль: Field не встал в конец левой стороны, и поднимать нечего");

  const cap = one(all(v.host, "io-side")[0] as StubNode, "io-side__cap");
  const moved = rowsOf(v.host).find(r => nameIn(r) === "Due") as StubNode;
  dragToSide(moved, cap);
  assert.deepEqual(layout(v.host), { left: ["Due", "Status"], right: [] },
    "бросок на подпись стороны обязан ставить Field первым, а не в конец");
  ok("бросок на подпись стороны ставит Field перед первой строкой");
}
...
{
  /*
   * **Его слово 2026-09-13** (`Скриншоты`): «при field=emoji в шапке стоит
   * emoji — сделай, чтобы у всех был хедер Values». Прежде здесь требовалось
   * обратное: у Field типа `element` колонка называлась именем типа, потому
   * что списка значений у него нет. Шапка, однако, называет **колонку**, а не
   * содержимое, и от смены Field в списке её имя прыгало.
   *
   * Тип Field при этом никуда не делся — он стоит чипом рядом с именем, и
   * рядом стоит контроль на это: иначе «у всех Values» выполнялось бы и у
   * панели, которая тип потеряла совсем.
   */
  const v = makeView();
